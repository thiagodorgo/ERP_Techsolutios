...
async function renderClientes(permissions: readonly string[]): Promise<string> {
  ...
  const { ClientesPage } = await import("../src/modules/registry/customers/pages/ClientesPage");
...
test("clientes page renderiza cabecalho, acao de criar e estado vazio (D-007)", async () => {
  const html = await renderClientes(["customers:read", "customers:create", "customers:update"]);

  assert.match(html, /Clientes/);
  assert.match(html, /Novo cliente/);
  assert.match(html, /Nenhum cliente cadastrado/);
  // D-007: modo mock nao fabrica linhas.
  assert.doesNotMatch(html, /Transportes Aurora|cus-1/);
});

test("clientes page esconde 'Novo cliente' sem permissao de criacao", async () => {
  const html = await renderClientes(["customers:read"]);
