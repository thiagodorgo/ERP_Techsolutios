...
async function renderEquipes(permissions: readonly string[]): Promise<string> {
  ...
  const { EquipesPage } = await import("../src/modules/registry/teams/pages/EquipesPage");

  setStoredAuthSession(mockSessionForEmail("gestor.web@techsolutions.example"));
  seedContext(permissions);

  return renderToString(
    <MemoryRouter initialEntries={["/cadastros/equipes"]}>
      <AuthProvider>
        <TenantProvider>
          <PermissionProvider>
            <EquipesPage />
...
test("equipes page renderiza cabecalho, acao de criar e estado vazio (D-007)", async () => {
  const html = await renderEquipes(["teams:read", "teams:create", "teams:update"]);

  assert.match(html, /Equipes/);
  assert.match(html, /Nova equipe/);
  assert.match(html, /Nenhuma equipe cadastrada/);
  // D-007: modo mock nao fabrica linhas.
  assert.doesNotMatch(html, /team-1|Guincho Zona Sul/);
});

test("equipes page esconde 'Nova equipe' sem permissao de criacao", async () => {
  const html = await renderEquipes(["teams:read"]);

  assert.match(html, /Equipes/);
  assert.match(html, /Nenhuma equipe cadastrada/);
  assert.doesNotMatch(html, /Nova equipe/);
});
