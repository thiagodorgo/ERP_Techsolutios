...
const CADASTROS_PATHS = ["/cadastros/clientes", "/cadastros/viaturas", "/cadastros/equipes", "/cadastros/servicos"];

const REGISTRY_PERMISSIONS = ["customers:read", "vehicles:read", "teams:read", "service_catalog:read"];
...
test("cadastros: Gestor com permissoes de leitura ve todas as 4 telas do menu Cadastros (A5)", async () => {
...
test("cadastros: rotulo do grupo 'Cadastros' resolve e agrupa as 4 telas (A5)", async () => {
  const { filterNavigationItems } = await import("../src/navigation/types");
  const { tenantNavigation } = await import("../src/navigation/tenantNavigation");
  ...
  assert.equal(registryGroup.items.length, 4);
  assert.deepEqual(
