...
import { listStockMovementsFromApi } from "./inventory.service";
import type { StockMovementsData, StockMovementsFilters } from "./inventory.types";

export function useStockMovements(filters: StockMovementsFilters) {
  const { session } = useAuth();
  const { activeContext } = useTenantContext();
  const [data, setData] = useState<StockMovementsData>({ items: [], pagination: { limit: 20, offset: 0, total: 0 }, source: "api" });
  ...
  const refresh = useCallback(async (background = false) => {
    ...
    setError(null);
    const nextData = await listStockMovementsFromApi(context, filters);
    setData(nextData);
