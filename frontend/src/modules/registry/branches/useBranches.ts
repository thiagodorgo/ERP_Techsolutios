...
import { listBranchesFromApi } from "./branches.service";
import type { BranchesData, BranchesFilters } from "./branches.types";

// Hook de listagem de Filiais. Carrega a janela (limit) uma vez; busca/situação/
...
export function useBranches(filters: BranchesFilters) {
  ...
  const [data, setData] = useState<BranchesData>({ items: [], pagination: { limit: 20, offset: 0, total: 0 }, source: "api" });
  ...
  const refresh = useCallback(async (background = false) => {
    ...
    setError(null);
    const nextData = await listBranchesFromApi(context, filters);
    setData(nextData);
