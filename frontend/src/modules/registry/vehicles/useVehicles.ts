...
import { listVehiclesFromApi } from "./vehicles.service";
import type { VehiclesData, VehiclesFilters } from "./vehicles.types";

export function useVehicles(filters: VehiclesFilters) {
  const { session } = useAuth();
  const { activeContext } = useTenantContext();
  const [data, setData] = useState<VehiclesData>({ items: [], pagination: { limit: 20, offset: 0, total: 0 }, source: "api" });
  ...
  const refresh = useCallback(async (background = false) => {
    ...
    setError(null);
    const nextData = await listVehiclesFromApi(context, filters);
    setData(nextData);
