...
import { listCustomersFromApi } from "./customers.service";
import type { CustomersData, CustomersFilters } from "./customers.types";

export function useCustomers(filters: CustomersFilters) {
  const { session } = useAuth();
  const { activeContext } = useTenantContext();
  const [data, setData] = useState<CustomersData>({ items: [], pagination: { limit: 20, offset: 0, total: 0 }, source: "api" });
  ...
  const refresh = useCallback(async (background = false) => {
    ...
    setError(null);
    const nextData = await listCustomersFromApi(context, filters);
    setData(nextData);
