...
import { listPoisFromApi } from "./pois.service";
import type { PoisData, PoisFilters } from "./pois.types";

// Hook de listagem de Pontos de Interesse. Carrega a janela (limit) uma vez; busca/situação/
// ordenação/paginação são client-side na dense-list (mesmo padrão dos Cadastros irmãos).
export function usePois(filters: PoisFilters) {
