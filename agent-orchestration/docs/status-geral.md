...
### Atualização 2026-09-02 — B-O6R-07a (autorização: SEC-002 P0 fechado, residuais do SEC-003 fechados, chave provisionada)

Sub-bloco `fix/o6r07a-authorization` (o 07b, do `Ω6R-SEC-004`/uploads, **não** entra aqui e o gate da CHECKLIST P1 só cai com os dois): `POST /approvals/:id/approve|reject` passam a exigir a chave DEDICADA `work_orders:approve` — antes usavam `work_orders:update`, que o técnico de campo tem —, o solicitante deixa de decidir a própria aprovação (**403 `self_decision`**, com rastro) e ator de campo só muta a OS **atribuída a ele** (**403 `not_assigned_to_actor`**), o que **cumpre** `RBAC_MATRIX.md:45` (`execute/update-assigned`) que o 200 anterior contrariava. No auth: falha anônima passa a armar o lockout atômico do B-O6R-01 com rastro (resposta segue **401 uniforme**), balde por **IP** nas duas rotas de login (**429 `RATE_LIMITED`**) e pino de `N/r/p` no parse do scrypt de tenant. A chave nova é **provisionada por migração** (`20260871000000`) porque produção roda só `migrate deploy`, sem seed — sem ela a permissão nasceria morta e a rota daria 403 para todos, inclusive o manager; foi esse achado que gerou a **EMENDA E1** do plano (o §5 proibia `prisma/**` e o §3.10 nomeara o instrumento errado), e a **EMENDA E2** consertou o **arranjo** de um teste em vez da asserção, para não mutilar a única prova HTTP da porta do conjunto.
Suíte canônica de execução real: **255 arq · 2647 testes · pass 2645 · fail 0 · skipped 2**, `ec=0` (Δ +36, todos dos 7 arquivos novos, com **vermelho-controle registrado em cada correção**); `check`/`lint`/`build`/`frontend check`/`frontend build`/`diff --check` todos `ec=0`; contrato mobile 25/25. `Ω6R-SEC-002` e `Ω6R-SEC-003` fechados **na autoria** — o painel os põe em `aguardando_merge` e **não** move `p0_fechados` (4) nem `p1_fechados` (0); veredito **REPROVADO PARA PRODUÇÃO** intacto. Fecham `P-O6R-B07A-PROVISIONAMENTO-DA-CHAVE`, `P-O6R-B07A-STICKY-409-VIRA-403` (com **correção de rumo**: o sticky voltou a 409, não virou 403), `P-O6R-B01-ANONIMO-SEM-LOCKOUT` e `P-O6R-B01-RATE-LIMIT-IP`; nascem `P-O6R-B07-APPROVAL-BY-POLICY` e `P-O6R-B07-RATE-LIMIT-DISTRIBUIDO` (índice regenerado **pelo gerador**: 244 → **249** cabeçalhos, ABERTAS 194 → **195**, FECHADAS 50 → **54**). **Dívida do #368 PAGA aqui** pela regra do primeiro-que-merge (`pr` 368 · `merge_commit` `f895dd2` · `approved_head` `d90fbbb`, o head da ata) e `blocks_completed` **157 → 158**; o ciclo 5 do `B-O6R-02` deve VERIFICAR e não duplicar. `mvp_demo`/`mvp_vendavel` **intocados**; `pr`/`merge_commit`/`approved_head` deste bloco **null na autoria** (§C3.5).
### Atualização 2026-09-03 — B-O6R-07a CICLO 2 (reprovado 2×1 no ciclo 1; ÚLTIMA tentativa — reversão do SEC-002, cobrança única, dual-match)

A junta do ciclo 1 REPROVOU 2×1 e o apenso `## CICLO 2` (C2·0–C2·8) governa a última tentativa (`D-TETO-DOIS-CICLOS`). Produto: a cobrança do lockout anônimo vira **ATO ÚNICO pós-veredicto** (1 requisição falhada = 1 incremento + 1 linha, agora com `ipAddress`/`userAgent` — fecha `P-O6R-B07A-RASTRO-ANONIMO-SEM-IP`; uso correto numa org não tranca mais a irmã; sucesso não fabrica auditoria de falha) e o guard de objeto ganha **DUAL-MATCH** (perfil OU user id — o 403 ao técnico legitimamente atribuído era defeito que o próprio ciclo 1 criou; `Ω6R-QUA-004` segue aberto). Migração `20260871000000`: só cabeçalho `--` com os dois avisos da junta; idempotência re-provada nos DOIS estados.
Registro: `Ω6R-SEC-002` **REVERTE** de "fechado" para **`parcialmente_superado`** com as 9 rotas abertas NOMEADAS (3 execução · 4 leitura · 2 condicionadas a env; origens `bf456b0` #173 e `D-Ω3F-5-COMMENT`), dona `P-O6R-SUBRECURSO-OBJECT-SCOPE` (ALTA, `B-O6R-07c`); SEC-002 sai de `aguardando_merge` (SEC-003 fica); `p0_fechados` segue **4**. Suíte canônica de execução real: **256 arq · 2656 testes · pass 2654 · fail 0 · skipped 2**, `ec=0` (Δ +9 por arquivo: multiorg NOVO 5 · -db +1 · object-scope +3, todos com vermelho-controle registrado); focados N=3 verdes; nascem `P-AUTH-KDF-ROTACAO-V2` (MÉDIA) e `P-KPI-HISTORY-MD-BACKLOG` (BAIXA) — índice regenerado **pelo gerador**: 249 → **252** cabeçalhos, ABERTAS 194 → **197**, FECHADAS 55. `mvp_demo`/`mvp_vendavel` **intocados**; `pr` 369; `merge_commit`/`approved_head` **null na autoria** (§C3.5).
