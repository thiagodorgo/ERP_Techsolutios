...
- **status:** FECHADA em 2026-09-05 · **severidade:** MEDIA · **fechado por:** `B-O6R-02` ciclo 5 (PR #371, `99f1840`) — o achado `Ω6R-DIN-001` que esta entrada virou está `fechado` com hash de merge no `achados.jsonl` · **evidência — a base forte primeiro:** `Ω6R-DIN-001.evidencia_fechamento` é de **EXECUÇÃO** — corrida nas DUAS ordens de disparo, **N=20 por ordem**, em memória **e** sob Postgres real, perdedor 422 sem lançamento órfão, saldo líquido 0, zero `40P01` — com **suíte permanente** `tests/financial-pay-title-atomic-db.test.ts`, roteada no job `backend-postgres` do `ci.yml` e verde no head deste PR. O código apenas **confirma** o mecanismo: `src/modules/financial-uow/financial-uow-prisma.ts` abre UMA transação tenant-scoped e entrega repositórios ligados a ela, e o comentário do módulo nomeia o achado — *"o work lançar = rollback de tudo — inclusive do lançamento do perdedor numa corrida (DIN-001, F3)"*. Era o fix que esta entrada pedia por extenso (*"envolver entry.create + applyPayment em `prisma.$transaction`"*) · **dono:** encerrado
  <sub>**Disposição de 2026-09-05** (ressalva do `porteiro-pos-merge` de `ed0a692`, achada por cadeira independente): a guarda-chuva `P-O6R-B02` — que o §Reconciliação dela declarava dona desta entrada, dando-lhe *"a severidade e o dono (bloco) que não tinham"* — **FECHOU** em 2026-09-05. Sem esta disposição, esta entrada ficaria ABERTA **sem dono nenhum**, com a matéria já fechada. Quem achou não escreveu esta disposição (§C7.4-bis).</sub>
  <sub>Triagem SAN2-1 (2026-08-29): a entrada não trazia linha de status. Marcada **ABERTA por padrão conservador** — não fechei o que não verifiquei. Ver `pendencias-indice.md`.</sub>
...
- **status:** FECHADA em 2026-09-05 · **severidade:** MEDIA · **fechado por:** `B-O6R-02` ciclo 5 (PR #371, `99f1840`) — `Ω6R-DIN-008` está `fechado` com hash de merge no `achados.jsonl` · **evidência — a base forte primeiro:** `Ω6R-DIN-008.evidencia_fechamento` é de **EXECUÇÃO** (writer bloqueia na trava SHARED e re-valida dentro da tx que grava; barreira determinística escopada por `application_name`, com controle negativo próprio), com **suíte permanente** `tests/financial-period-close-write-race-db.test.ts` no job `backend-postgres` do `ci.yml`, verde no head deste PR. O código confirma o alcance: esta entrada dizia que o fix REAL exigia o guard-read do writer NA MESMA tx, com lock compartilhado em `(tenant,period)`, e que isso era "fora deste bloco". Hoje `src/database/financial-period-lock.ts` é o lar único (`pg_advisory_xact_lock` + `..._shared`) e `src/modules/financial-titles/financial-title-prisma.repository.ts` **importa e usa** `acquirePeriodLockShared` via `assertPeriodOpenSharedInTx` — usado por títulos, por lançamentos e, pelo `uow.ctx`, por cheques e lançamentos de serviço: **são os write-paths Ω4-2..4** que esta entrada dava como fora do bloco. O comentário de `financial-period-close-prisma.repository.ts` afirma nominalmente que *"o read-skew de P-Ω4-6-CLOSE-RACE está fechado"* · **dono:** encerrado
  <sub>**Disposição de 2026-09-05** (ressalva do `porteiro-pos-merge` de `ed0a692`, achada por cadeira independente): a guarda-chuva `P-O6R-B02` — que o §Reconciliação dela declarava dona desta entrada, dando-lhe *"a severidade e o dono (bloco) que não tinham"* — **FECHOU** em 2026-09-05. Sem esta disposição, esta entrada ficaria ABERTA **sem dono nenhum**, com a matéria já fechada. Quem achou não escreveu esta disposição (§C7.4-bis).</sub>
  <sub>Triagem SAN2-1 (2026-08-29): a entrada não trazia linha de status. Marcada **ABERTA por padrão conservador** — não fechei o que não verifiquei. Ver `pendencias-indice.md`.</sub>
...
- **AMPLIAÇÃO NOMINAL DECLARADA (`B-SAN3-00`, 2026-09-21 — dívida 3 do parecer do porteiro pós-merge do #390):** o `prisma/seed.ts` **estava** no §5 do `B-SAN3-07`, mas **autorizado só para o nome** — semear cinco papéis é outra mudança no mesmo arquivo, e o dono seria nominal sem a autorização. A linha do `B-SAN3-07` no §5 passa a autorizar o **mesmo** arquivo **também** para semear `finance`, `inventory`, `operator`, `field_technician` e `support`, **aditivo**: nenhum papel existente é removido e nenhuma concessão é retirada. Dono real **confirmado**: `B-SAN3-07`. **QUÓRUM DO `B-SAN3-07` SUBIU JUNTO (pré-merge do #392, achado `C1-A5` da junta do `B-SAN3-00`):** com esta ampliação o bloco passa a semear **papéis globais com as concessões do catálogo**, logo toca **permissão**, e a coluna Junta dele no §5 foi de `maioria` para **`unanimidade + coordenador-de-acessos`** (§C7.1-ter(b)). Alargar a fronteira sem mexer no quórum era o descompasso que ESTE bloco criou e fechou.
...
- **AMPLIAÇÃO NOMINAL DECLARADA (`B-SAN3-00`, 2026-09-21 — dívida 3 do parecer do porteiro pós-merge do #390):** o `B-SAN3-06a` era dono **sem ter o arquivo do conserto na fronteira** — `frontend/src/modules/auth/auth.adapter.ts` não estava no §5 do `docs/revisoes/SAN3/PLANO_SAN3.md` para bloco nenhum, a mesma forma do achado **A2** do porteiro do #387 (dono nomeado sem o arquivo no §5). A linha do `B-SAN3-06a` no §5 passa a declará-lo pelo nome, **restrita ao mapa estático `rolePermissions` (l.260-356)** — a dívida deixa de ser promessa de "quem executar declara" e vira fronteira escrita. Dono real **confirmado**: `B-SAN3-06a`.
...
- **dono:** `B-SAN3-06a` (proposto pela C2 — já é dono de `P-SAN3-04A-MENU-RESIDUAL` e de `P-SAN3-04A-FRONT-PERMISSOES-POR-PAPEL-DEFASADAS`, e o §5 do `PLANO_SAN3.md` l.270 lhe dá `appSidebarNav.ts` e `tenantNavigation.ts`, os arquivos que decidem o menu). **Ampliação nominal DECLARADA** (`B-SAN3-00`, 2026-09-21): `docs/navigation-matrix.md` não estava no §5 de bloco nenhum. Em vez de deixar a ampliação como promessa ("quem executar o `06a` a declara"), ela entrou na **linha do `B-SAN3-06a` no §5** do `PLANO_SAN3.md`, junto com `auth.adapter.ts` — mesma classe de dívida, mesmo bloco, mesma passada. O precedente do `frontend/index.html` na `P-WEB-FONTE-INTER-NAO-CARREGADA` continua valendo para quem ainda não tem bloco dono.
...
- status: ABERTA (nomeada pelo próprio `B-SAN3-B1`, que **fixou** a versão do CI mas **não** resolve a divergência — estava fora do escopo permitido do bloco)
- **prova (N = 2 versões divergentes; forma: SDK resolvido no CI × SDK instalado na máquina do dono; causa: o CI usava `channel: stable` sem versão e o canal serve o que for mais novo no dia):** medido por execução própria no caminho `/opt/hostedtoolcache/flutter/stable-<versão>-x64` dos logs do CI — `3.47.4` até **2026-09-18T18:37Z** (runs `35380369290`, `35381265287`) e `3.47.5` a partir de **2026-09-19T00:26Z** (run `35409352362`). A máquina do dono tem **Flutter 3.41.6 / Dart 3.11.4**. A deriva **aconteceu dentro da rodada SAN3** e foi dela que veio a divergência de `dart format` que deixou o CI vermelho no **#388**.
- **escopo:** `pre-existente` — evidência de data/origem: o job `flutter` do `.github/workflows/ci.yml` está com `channel: stable` **sem** `flutter-version` desde antes desta rodada, e a versão da máquina do dono é anterior a qualquer bloco SAN3. O `B-SAN3-B1` **não criou** a divergência: ele a tornou **explícita e imóvel** do lado do CI.
- **o que o `B-SAN3-B1` fechou e o que NÃO fechou.** FECHOU: o CI parou de derivar sozinho — `flutter-version: 3.47.5` fixado, e subir a versão passa a ser **ato deliberado**, visível no diff e julgado pela junta. NÃO FECHOU: o dono continua rodando `dart format`/`flutter analyze` numa versão diferente da do CI, então uma formatação válida localmente ainda pode sair vermelha no CI (e vice-versa).
- **dono:** **`B-SAN3-A2`** — o `terreno preflight` desse bloco é quem avisa a divergência ao operador. A decisão de **qual** das duas versões converge (subir a da máquina do dono ou baixar a do CI) é do dono: mexer na máquina dele não é ato de agente.
...
- **teste de encerramento:** `flutter --version` na máquina do dono e o `flutter-version` do `ci.yml` devolvem a **mesma** versão; vermelho-controle: divergi-las de propósito faz o `terreno preflight` do `B-SAN3-A2` avisar. **Ou** o dono decide conviver com a divergência, e aí ela vira decisão registrada em `controle/decisoes.md` em vez de pendência.
...
- status: ABERTA (`B-SAN3-00`, medição do dev; a **prescrição original do mandato foi falsificada** — registro §A2 em `decisoes.md`, `REGISTRO-SAN3-00-CORPOS-DE-JURADO`)
- **prova (N = 33 cadeiras; forma: corpo de agente presente no disco de `.claude/agents/especialistas/` da árvore principal, cuja identidade já foi SEPULTADA no `OBITUARIO-IDENTIDADES.md` e/ou APOSENTADA em `controle/aposentadoria-especialistas.md`; causa: as rodadas 1–3 de aposentadoria foram aplicadas na `main`, e a árvore principal está em `demo/investidor`, onde elas nunca chegaram):** varredura exaustiva `git rev-list --objects --all` (314 blobs sob `.claude/agents/**` + `.agents/agents/**`) × `git hash-object` de cada corpo do disco. Dos **129** corpos no disco, **80** estão em caminhos que a `origin/main` não rastreia — **41** cadeiras no espelho `.claude/`, **39** no `.agents/`. Classificação dos 41, um a um: **10** sepultadas e aposentadas · **13** só sepultadas · **10** só aposentadas = **33**; as **8** restantes são as cadeiras dos blocos **em voo** (`jurado-o6r04a-c2-*` no #389/`738ff531` e `jurado-o6r11-*` no #388/`43557a17`), e não são resíduo.
- **durabilidade: NÃO há perda.** **80 de 80** têm blob alcançável em alguma ref (0 perdido), e os 8 em voo são **byte-idênticos** ao blob da branch que a junta julga (14 comparações por hash, 14 iguais). Esta pendência é de **higiene de contexto**, não de recuperação.
- **efeito medido:** enquanto os 33 estiverem no diretório vivo daquela árvore, o registro de agentes os resolve e o `description` de cada um pesa no contexto de **toda sessão** aberta a partir dela — exatamente o custo que a `D-APOSENTADORIA-ELENCO-EFEMERO` mediu e removeu na `main`. A `OBITUARIO-IDENTIDADES.md` §1.2 é literal: *"Identidade SEPULTADA não entra em junta nenhuma. Nunca."* — e um corpo presente no diretório vivo pode ser invocado.
- **escopo:** `pre-existente` — as três rodadas de aposentadoria são de 2026-09-07, 2026-09-08 e 2026-09-17, todas anteriores a este bloco; o `B-SAN3-00` não criou nenhum desses arquivos e o seu escopo permitido não inclui a árvore de `demo/investidor`.
- **por que o dev NÃO decidiu sozinho:** apagar arquivo do diretório vivo de **outra** árvore de trabalho é varrer resíduo alheio, que se **reporta** e não se varre (`feedback-remocao-por-identificador-de-bloco`: em 04/09 uma cadeira destruiu o worktree vivo de outra sessão lendo o nome como dela). A árvore principal está em `demo/investidor@d1fab3bc`, divergente da `main`, e tem **4 arquivos com ` M` real** de outra sessão (apensos do ciclo 5) — mexer ali agora colide com trabalho vivo de terceiro.
- **proposta do dev (a junta decide, não o dev):** a remoção seja feita **na própria árvore de `demo/investidor`**, por quem for dono daquela branch, **por identificador de BLOCO** (`jurado-c4-`, `jurado-c5-`, `jurado-arnes-`, `jurado-06-`, `jurado-07b-`, `critico-c5-`/`suplente-critico-c5-`), **nunca por nome de cadeira**, e **só depois** de os PRs #388 e #389 mergearem — porque antes disso as 8 cadeiras em voo convivem no mesmo diretório e uma varredura por diretório as levaria junto. Alternativa igualmente aceitável: nada se apaga e a branch `demo/investidor` é reconciliada com a `main`, o que resolve as 33 de uma vez.
- **dono:** **a junta do `B-SAN3-00` nomeia.** O dev **não** propõe bloco dono: nenhum bloco do §5 do `docs/revisoes/SAN3/PLANO_SAN3.md` tem `.claude/agents/**` da árvore de `demo/investidor` na fronteira, e inventar um dono aqui seria a mesma falha que o achado **A2** do porteiro do #387 nomeou (dono sem o arquivo no §5).
- **bloqueia:** **não bloqueia o gate** nem o próximo bloco — é peso de contexto, sem efeito sobre código, dado ou permissão.
- **teste de encerramento:** na árvore de `demo/investidor`, `ls .claude/agents/especialistas/` não lista nenhuma identidade que o `OBITUARIO-IDENTIDADES.md` declare sepultada nem que a `aposentadoria-especialistas.md` declare aposentada; `node scripts/sync-agent-agents.mjs --check` verde nos dois espelhos; e **nenhum blob some** — `git rev-list --objects --all` continua alcançando os 80 corpos medidos aqui.
...
- status: ABERTA (aberta pelo **pré-merge do `B-SAN3-00`** a partir do achado **C2-A2** da junta, `agente-secops`, gravidade `nota`; a classe **não** reprova o bloco e **nada regrediu**)
...
- **prova (N = 88 sondas, 86 escondidas e 2 visíveis; forma: `git check-ignore -q --no-index <caminho>` — a régua é o **exit code**, nunca a saída de `-v`, que imprime também o padrão de negação —, com a classe **GERADA DA FONTE** e não por lista escrita à mão: os **22** padrões do `~/.config/git/ignore` que **não contêm barra** (e por isso casam em **qualquer profundidade**) × os **4** diretórios que o bloco reincluiu (`.claude/agents`, `.claude/skills`, `.agents/agents`, `.agents/skills`); causa: a reinclusão do `.gitignore` do repositório desfaz o ignore **do DIRETÓRIO** (`.claude/`, l.2; `.agents/`, l.27), mas **não** os ignores **por NOME**, que continuam casando lá dentro — e o bloco abriu exceção nominal para **um** nome só, `SKILL.md`):** as **2** visíveis são exatamente `.claude/skills/**/SKILL.md` e `.agents/skills/**/SKILL.md`, a exceção que o bloco declarou (`.gitignore:72-73`). As **86** escondidas incluem `CLAUDE.md` (global l.3), `CLAUDE.local.md` (l.4), `AGENTS.md` (l.5), `GEMINI.md` (l.6), `.cursorrules` (l.9), `.cursorignore` (l.10), `.mcp.json` (l.14), `.windsurfrules` (l.16), `.aider*` (l.17), `.clinerules*` (l.21), `*.agent.md` (l.24), `AGENT.md` (l.25), `CODEX.md` (l.28), `QWEN.md` (l.29), `CRUSH.md` (l.30), `WARP.md` (l.31), `OPENCODE.md` (l.32), `COPILOT.md` (l.33), `SKILL.md` (l.34), `*.prompt.md` (l.35), `*.instructions.md` (l.36) e `*.chatmode.md` (l.37) — e, entre elas, `SKILL.md` **dentro de `agents/`**, porque a exceção do bloco é restrita a `skills/**`.
- **divergência de N declarada, não escolhida:** a cadeira **C2** publicou **8/8** sondas (6 nomes × o espelho `.claude/` + 2 equivalentes no `.agents/`), escritas à mão; o pré-merge **gerou a classe do arquivo-fonte** e mediu **88**, das quais **86** escondidas. **O veredito é o mesmo** — a classe residual existe e não fechou inteira —, e o denominador do pré-merge é o maior porque cobre os 22 padrões nos 4 diretórios em vez de 6 nomes em 2. Publicado com N, forma e causa, como a regra manda.
- **escopo:** `pre-existente` — evidência de origem: o `~/.config/git/ignore` é arquivo da **máquina do dono**, fora do repositório, e os 22 padrões já estavam lá antes deste bloco; os 86 caminhos sondados **já eram invisíveis na base**, onde os diretórios inteiros eram ignorados. O `B-SAN3-00` **não criou** a classe — ele fechou a parte do diretório e a parte de `SKILL.md`, e é por tê-las fechado que o resto ficou visível como classe.
- **efeito medido: NENHUM sobre o que está rastreado hoje.** Pela medição falsifiável do pré-merge (universo único, **3501** caminhos rastreados do objeto), os rastreados ignorados são **3** — `AGENTS.md`, `CLAUDE.md` e `docs/claude-code-handoff/CLAUDE.md` —, **nenhum** deles dentro de `.claude/` ou `.agents/`, e **os três já eram ignorados na base** (`comm -23` = **0** passaram a ser ignorados). O risco é **prospectivo**: um arquivo **novo** com um desses nomes dentro dos diretórios reincluídos nasce invisível como `??`, que é **exatamente o mecanismo** do achado A1 do porteiro do #387 — a cadeira que reprovou o ciclo 2 do `B-SAN3-01` votou sem estar em commit nenhum.
- **dono: A NOMEAR pelo orquestrador/junta — o pré-merge PROPÕE e justifica, e não decide.** Proposta: um bloco novo de registro no §5 do `docs/revisoes/SAN3/PLANO_SAN3.md` (sugestão de identificador: `B-SAN3-00b` · `chore/gitignore-classe-residual`), com `.gitignore` na fronteira. **Por que não nomear um bloco existente:** `grep -n gitignore docs/revisoes/SAN3/PLANO_SAN3.md` no head deste PR devolve **zero** linhas — **nenhum** bloco do §5 tem `.gitignore` na fronteira, e o único que o teve foi este `B-SAN3-00`, que está fechando. Nomear outro repetiria o achado **A2** do porteiro do #387 (dono sem o arquivo no §5), que é justamente o defeito que a dívida 3 deste PR veio consertar. **Alternativa que não é ato de agente:** o dono edita o próprio `~/.config/git/ignore`; mexer na máquina do dono não é ato de agente, e por isso a forma recomendada é a do repositório.
...
## P-GOV-MANDATO-PREFLIGHT-CAMINHO-POR-BASENAME (2026-09-25) — a checagem de caminho do pré-voo discrimina por NOME DE ARQUIVO, não por caminho: um caminho errado com basename real passa — BAIXA

- status: FECHADA em 2026-09-26, no **ciclo 2 do `B-GOV-MANDATO` (PR #393)**. Dono nomeado pela ata
  `J-B-GOV-MANDATO.md` (achado C3-02), que mandou o próprio ciclo 2 fechá-la **junto com a classe** — a
  classe é "guarda que reconhece FORMA CONHECIDA em vez de enunciar a PROPRIEDADE", e o basename era a
  terceira instância dela. **Teste de encerramento executado, verde, em `tests/mandato-preflight.test.ts`:**
  caso `[B6]` — 20 basenames **gerados da fonte** por `git ls-files 'scripts/*.mjs' 'tests/*.ts'
  'src/config/*.ts'`, citados sob `src/diretorio/que/nao/existe/` → **20 de 20 REJEITADAS** (eram 20 de 20
  aceitas); caso `[B6b]` — `lib/core/sync/sync_action_store.dart` → **aceito**, o uso legítimo relativo à
  raiz do app Flutter continua de pé; caso `[B6c]` — 5 basenames inexistentes → **5 rejeitadas**, o
  vermelho-controle segue vermelho. Mutação que reabre o defeito: reintroduzir
  `find "$RAIZ" -name "$(basename "$c")" … -print -quit && continue` deixa `[B6]` vermelho.
- **divergência declarada quanto à "forma do conserto" proposta abaixo:** o conserto NÃO foi o
  `find -path "*/$c"`. O plano do ciclo 2 (`docs/revisoes/SAN3/B-GOV-MANDATO-ciclo2-plano.md`, E2 checagem 6)
  mandou **eliminar a busca por nome** e exigir existência no caminho citado, conferida em `$RAIZ/<c>` **ou**
  em `$RAIZ/mobile/flutter_app/<c>` — o que preserva o uso Flutter sem manter casamento por sufixo. Custo
  novo, declarado no cabeçalho do script e com dono em `P-GOV-MANDATO-2-FRONTEIRAS`: caminho **sem extensão
  e sem barra final** deixa de ser conferido, porque pela estrutura ele é indistinguível de `e/ou` e `14/14`.
- status histórico (2026-09-25, ciclo 1): ABERTA (`B-GOV-MANDATO`, medição do **dev**, achada ao derrubar a 3ª hipótese do mandato — a hipótese era sobre falso **positivo** e não caiu; esta é a propriedade **vizinha**, o falso **negativo**, que a sonda encontrou de passagem)
- **prova (N = 20 sondas, 20 aceitas; forma: mandato sintético mínimo — seções `MEDIDO`/`HIPOTESE` bem-formadas — citando um caminho `src/diretorio/que/nao/existe/<basename>` onde `<basename>` é o nome de um arquivo **realmente rastreado** (classe GERADA DA FONTE por `git ls-files 'scripts/*.mjs' 'tests/*.ts' 'src/config/*.ts' | head -20`, não por lista escrita à mão), submetido a `bash scripts/mandato-preflight.sh <sonda>`, régua = **exit code**; causa: a checagem 6 do script tenta, nesta ordem, `$RAIZ/$c`, `$RAIZ/mobile/flutter_app/$c`, prefixo de unidade Windows e, por último, `find "$RAIZ" -name "$(basename "$c")" -print -quit` — e é esta última cláusula que aceita o caminho errado, porque `find -name` casa o **nome**, não o caminho):** **20 de 20** aceitas (`PRE-VOO OK`, exit 0).
- **vermelho-controle (N = 5, 5 rejeitadas):** o mesmo diretório inexistente com um basename que **não existe em lugar nenhum** (`arquivo_inexistente_zzz{1..5}.ts`) é **rejeitado** nas 5 (`caminho citado nao existe em lugar nenhum do repo`, exit 1). Ou seja: a checagem **funciona** e **discrimina** — só que a propriedade que ela decide é *"existe um arquivo com este nome"*, e não *"existe um arquivo neste caminho"*. Sem este par, "20 aceitas" não provaria nada.
- **escopo:** `dentro-do-bloco` — `scripts/mandato-preflight.sh` é arquivo **novo** deste bloco (`git diff --name-status fc3363e3 1ae82626` marca `A`), logo a classe nasce aqui e não tem dono anterior. Declarado como tal em vez de empurrado para `pre-existente`.
- **por que NÃO bloqueia, e por que mesmo assim vira pendência:** a checagem é um **filtro de sanidade** de mandato, não uma defesa de segurança, permissão, dinheiro ou dado — nada no produto depende dela, e o efeito do falso negativo é deixar passar um mandato que cita um caminho com o diretório errado. O que ela **já pega** continua pego: caminho inventado de ponta a ponta (o caso que motivou a checagem, e que o vermelho-controle confirma). Registrar em vez de consertar dentro do bloco segue o §C7.4-bis: quem achou não conserta, e a correção é decisão de escopo — apertar o casamento tem custo real (ver abaixo).
- **por que a cláusula existe, e por que apertá-la não é trivial:** o `find` por basename é justamente o que faz o pré-voo aceitar caminho **relativo à raiz do app Flutter** citado como `lib/…` sem o prefixo `mobile/flutter_app/` — hábito corrente nos mandatos e que a 3ª hipótese deste mandato testou (e que **não** produz falso positivo: sonda medida, `PRE-VOO OK`). Trocar `-name` por casamento de **sufixo de caminho** (`find … -path "*/$c"`) preservaria esse uso e fecharia o buraco; trocar por exigência de caminho exato quebraria o uso legítimo e faria o pré-voo rejeitar mandato correto — que é o pior modo de falha para uma trava de saída.
- **dono:** `B-GOV-MANDATO` ciclo 2 — nomeado pela ata `J-B-GOV-MANDATO.md` (achado C3-02) em 2026-09-26; a proposta original do dev do ciclo 1, preservada abaixo, era: **A NOMEAR pela junta — o dev PROPÕE e justifica, e não decide.** Proposta: emenda ao próprio `B-GOV-MANDATO` numa passada posterior, **ou** um bloco de registro próprio; não há outro bloco do §5 do `docs/revisoes/SAN3/PLANO_SAN3.md` com `scripts/mandato-preflight.sh` na fronteira, e inventar um dono repetiria o achado **A2** do porteiro do #387 (dono sem o arquivo no §5).
- **forma do conserto, para quem pegar:** substituir a última cláusula da checagem 6 por casamento de **sufixo de caminho** — `find "$RAIZ" -path "*/$c" -not -path '*/node_modules/*' -print -quit` —, mantendo as três tentativas anteriores intactas (raiz, raiz do app Flutter, unidade Windows). Um guard em `tests/` que gere a classe da fonte (basenames reais sob diretório inexistente) e exija **0 aceitas**, com o vermelho-controle do caminho Flutter legítimo exigindo **aceita**, fecharia a propriedade em vez da instância.
- **bloqueia:** **não bloqueia** o gate nem o próximo bloco — sem efeito sobre código, dado, dinheiro ou permissão.
- **teste de encerramento:** a sonda gerada da fonte (≥20 basenames rastreados sob `src/diretorio/que/nao/existe/`) devolve **0 aceitas**; o caminho legítimo relativo ao app Flutter (`lib/core/sync/sync_action_store.dart`) continua **aceito**; e o vermelho-controle do basename inexistente continua **rejeitado** nas 5.

## P-GOV-ATA-CABECALHO-TEMPLATE (2026-09-26) — não existe template RASTREADO de ata com cabeçalho legível por máquina: 102 das 107 atas não declaram o objeto — MÉDIA

- status: ABERTA
- **dono:** `B-GOV-ATA-CABECALHO` (bloco novo, a nomear na fila do `docs/revisoes/SAN3/PLANO_SAN3.md` §7.3 pelo orquestrador — o dev do ciclo 2 do `B-GOV-MANDATO` não escreve nessa fila, §4 do plano do ciclo 2).
- **o que é:** a forma do cabeçalho da ata (`# J-… (PR #N)` no título e `- **Objeto…:** `sha`` em coluna 0) é **ritual da junta**, não da ferramenta. `scripts/mandato-refs.sh` passou a DECLARAR o que não consegue ler, em vez de adivinhar — mas quem faz a ata virar legível é o ritual.
- **medição (2026-09-26, pelo dev do ciclo 2):** censo em `origin/main@fc3363e3` — `total_atas=107`, `com_linha_Objeto=5`, `linha_estrutural_approved_head=0`. No head do ramo `chore/mandato-refs-e-preflight@9aa8fc7e`: `total_atas=108`, `approved_head=0`. Forma: laço sobre `git ls-tree -r --name-only <ref> agent-orchestration/omega/juntas/ | grep -iE '/J-.*\.md$'` com `git show "<ref>:$f" | grep -iqE '^\- \*\*(Objeto( julgado)?|approved_head):\*\*'`.
- **escopo:** `pre-existente` — as atas estão em `origin/main@fc3363e3` e nem o ciclo 1 nem o ciclo 2 do `B-GOV-MANDATO` alteram ata alheia (as 106 outras atas são escopo PROIBIDO no §4 do plano). É a mesma classe do achado C2-06 da ata do ciclo 1.
- **bloqueia:** **não bloqueia**. A ferramenta é fail-closed: sem cabeçalho legível ela devolve `NAO DETERMINAVEL` (ec=3) listando o que viu, em vez de afirmar `approved_head`.
- **teste de encerramento:** existe um arquivo RASTREADO de template de ata em `agent-orchestration/omega/juntas/` cujo esqueleto traz o título com `(PR #N)` e a linha `- **Objeto julgado:**`; e uma ata nova escrita a partir dele faz `bash scripts/mandato-refs.sh <PR>` sair de `NAO DETERMINAVEL` por "sem linha de Objeto". O `TEMPLATE-J-ata.md` visto na árvore principal em 2026-09-26 é de outra sessão, **não é rastreado** (`git ls-files` = 0) e **não tem** linha de Objeto — reportado, não tocado.

## P-GOV-ATA-APPROVED-HEAD-LINHA (2026-09-26) — nenhuma das 107 atas escreve a linha `- **approved_head:**`, logo o estado LIDO é INALCANÇÁVEL no corpus de hoje — MÉDIA

- status: ABERTA
- **dono:** `B-GOV-ATA-CABECALHO` (a nomear na fila §7.3 pelo orquestrador).
- **o que é:** `scripts/mandato-refs.sh` passou a ler `approved_head` **só** de uma linha em que a junta ESCREVE `- **approved_head:** `sha`` (adendo A1 do plano do ciclo 2). Ler o veredito em prosa seria reconhecer FORMA — a classe que este bloco existe para fechar: são **três grafias** (`- **Resultado: …`, `## VEREDITO: …`, `## Ciclo N — … — X`) em 10 das 107 atas, e numa delas APROVADO e REPROVADO estão na MESMA linha.
- **medição (2026-09-26, pelo dev do ciclo 2):** `linha_estrutural_approved_head = 0` de 107 em `origin/main@fc3363e3` e 0 de 108 no head do ramo (mesma forma do censo acima).
- **consequência DECLARADA e aceita:** os PRs #390, #391 e #392 deixam de sair como "lidos" e passam a `NAO DETERMINAVEL` com o objeto listado. Eles estavam certos **por sorte do veredito** — os três foram APROVADOS, e a ferramenta não sabia disso. Medido: `bash scripts/mandato-refs.sh 392` → ec=3, objeto `7822deaf9afabd076d1095eaf48a6dfb635e5401` em `agent-orchestration/omega/juntas/J-B-SAN3-00.md:3`, **sem** linha de aprovação.
- **o que fecha:** (a) o template EMITE a linha quando o veredito é APROVADO; (b) convenção para ata **multi-ciclo** (um arquivo por ciclo, ou a linha na seção do ciclo aprovado); (c) retrofit das 4 atas APROVADAS que já têm linha de Objeto — **opcional, decisão do dono**, porque tocar ata histórica não é decisão de agente.
- **bloqueia:** **não bloqueia** — `NAO DETERMINAVEL` com a lista do que foi visto é o produto desejado, não uma falha.
- **teste de encerramento:** uma ata do repositório com veredito APROVADO traz a linha `- **approved_head:**` coerente com um objeto que ela declara, e `bash scripts/mandato-refs.sh <PR>` devolve **LIDO, ec=0**, nomeando arquivo e linha — o primeiro LIDO vivo do repositório.

## P-GOV-MANDATO-2-FRONTEIRAS (2026-09-26) — as OITO fronteiras que o pré-voo e o refs DECLARAM não atravessar — BAIXA

- status: ABERTA
- **dono:** `B-GOV-MANDATO-2` (bloco novo, a nomear na fila §7.3 pelo orquestrador).
- **o que é:** o ciclo 2 moveu cada checagem para uma PROPRIEDADE e, em vez de esconder o que sobrou, declarou. Estas são as sobras, todas no cabeçalho dos dois scripts:
  1. **SHA dentro de URL** (`…/commit/7462b75b`): o token contém `/`, logo não é classificado como SHA e a proveniência não é conferida. É propriedade **vizinha** de "SHA citado como identificador".
  2. **`Select-String`/`findstr`**: a checagem 5 enuncia `grep`/`rg`; a casa usa bash nos mandatos. A forma longa `--ignore-case` também não é reconhecida — use `-i` ou `caixa-exata:`.
  3. **Nome de arquivo sem `/`** (`CLAUDE.md`, `package.json`): a tokenização de caminho exige `/`.
  4. **Caminho sem extensão E sem barra final** (`src/modules/work-orders`): não é conferido, porque pela estrutura é indistinguível de `e/ou` e de `14/14`. **Custo NOVO do ciclo 2**, introduzido ao matar a lista de 10 extensões e o `find -name` (fecha `P-GOV-MANDATO-PREFLIGHT-CAMINHO-POR-BASENAME`). Contorno: cite o diretório com barra final.
  5. **Drift da forma do JSON do `gh`**: a fixture do shim em `tests/mandato-refs.test.ts` é **datada** (2026-09-26T03:39Z, gh 2.89.0). Se o GitHub renomear um campo, só a execução viva da bateria pega.
  6. **A proveniência é um RETRATO, não um histórico.** A checagem 4 exige que todo SHA citado esteja na saída ATUAL do `mandato-refs.sh`. Um documento que registre a EVOLUÇÃO do head — "comecei em X, empurrei Y" — tem o X rejeitado assim que o ramo anda. **Medido no dogfooding do relatório do dev do ciclo 2**, que teve o próprio head de partida recusado com a mensagem "pode ser SHA VELHO: o ramo anda". O pré-voo está **certo** pela regra e pelo motivo; o contorno hoje é citar o head antigo abaixo de 7 hex. Fechar isto exigiria a ferramenta aceitar SHAs que são ANCESTRAIS do head do PR (`git merge-base --is-ancestor`), o que é decisão de escopo, não de conserto.
  7. **Rota de API citada num comando** (`gh api` sobre o endpoint de check-runs) é estruturalmente indistinguível de um caminho do repositório e cai na checagem 6. **Medido no mesmo dogfooding.** Distinguir exigiria reconhecer a FORMA do comando em volta — exatamente a classe que este bloco fecha —, então fica declarado em vez de remendado.
  8. **Um `md5` tem 32 caracteres hexadecimais** e cai DENTRO da faixa de 7 a 40 da checagem 4, logo é classificado como SHA e cobrado da proveniência do PR. A própria disciplina da casa pede md5 **neutro de fim de linha** em várias comparações (o `diff` cru mente no Windows), então isto aparece em mandato real — **medido no dogfooding do relatório do dev do ciclo 2**. Não foi consertado de propósito: distinguir exigiria olhar a palavra "md5" em volta, que é **vocabulário** — a classe que este bloco fecha. Contorno hoje: encurtar o md5 abaixo de 7 hex, ou parti-lo.
- **bloqueia:** **não bloqueia** — nenhuma delas tem efeito sobre código, dado, dinheiro ou permissão; todas são filtros de sanidade de mandato.
- **teste de encerramento:** para cada uma das OITO, um caso em `tests/mandato-preflight.test.ts` ou `tests/mandato-refs.test.ts` que hoje passaria despercebido fica **vermelho**, com o vermelho-controle do uso legítimo continuando **verde** (ex.: 4 — `src/modules/work-orders` rejeitado quando não existe, e `e/ou` continuando aceito).
- **correção de registro (orquestrador, 2026-09-26):** o título dizia "cinco" e o corpo lista **oito**; a numeração saía 1-6, **8, 7**. Achado do `inspetor-de-terreno-da-junta` da junta 2 (ressalva R3). Contagem e ordem corrigidas; **nenhuma fronteira foi acrescentada, removida ou reescrita** — conferível pelo diff.
