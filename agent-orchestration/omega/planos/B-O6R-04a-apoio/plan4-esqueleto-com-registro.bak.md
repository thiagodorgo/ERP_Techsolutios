...
> **Papel:** `planejador-mestre` · **Modelo que rodou:** Fable 5.1 (`claude-fable-5-1`, o fixado no frontmatter; sem fallback) · **Corpo aplicado:** `origin/main:.claude/agents/planejador-mestre.md` lido por `MSYS_NO_PATHCONV=1 git -C <worktree b04a> show origin/main:.claude/agents/planejador-mestre.md` `[00]`.
> **Status:** EM APURAÇÃO — esqueleto gravado em 2026-09-18; cada seção é preenchida à medida que a medição sai (P2).

## 0. Tabela da emenda 3 — achado → requisito → desenho → teste com vermelho-controle (cenário do crítico) → medição minha
EM APURAÇÃO

## 1. Objetivo · ator · fluxo origem→destino
EM APURAÇÃO

## 2. Mapa das vias (gerado do código) — com o estado da contagem e o `open` dentro
EM APURAÇÃO

## 3. Desenho
EM APURAÇÃO

## 4. Modelagem — migração aditiva fail-closed (M-01), censo, roteiro (M-02)
EM APURAÇÃO

## 5. Contrato
EM APURAÇÃO

## 6. Testes de encerramento — cada caso com vermelho-controle
EM APURAÇÃO

## 7. CE-G1 e CE-G2
EM APURAÇÃO

## 8. Escopo permitido e proibido — arquivo a arquivo
EM APURAÇÃO

## 9. Baseline N · meta M ≥ 2N · KPI no PR (K-01)
EM APURAÇÃO
...
## 13. Pendências que nascem (com dono) — e as que não nascem
EM APURAÇÃO
