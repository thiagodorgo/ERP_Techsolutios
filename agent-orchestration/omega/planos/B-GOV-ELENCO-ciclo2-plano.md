...
4. **Corpo carregado × corpo julgado (lacuna do ciclo 1):** o briefing declara **como** cada identidade é carregada (subagente nomeado do `.claude/agents/` do cwd da sessão, ou corpo colado do blob do head) e o inspetor confere, por identidade usada, comparação **EOL-neutra** — `git -C <wt> show <head>:.claude/agents/<x>.md` contra o arquivo carregado, ambos com `tr -d ''` antes do hash, ou `git -c core.autocrlf=false show`. **NUNCA `md5sum` cru dos dois lados** (ressalva R3 do inspetor da fatia A): a árvore de trabalho está sob `core.autocrlf=true` e chega ao disco com CRLF enquanto o blob tem LF — o hash cru **fabrica divergência** e bloquearia a fatia B por artefato de quebra de linha. É a lição do §C7.1-ter(c), que já custou uma pendência ALTA fechada por não-reprodução. Divergência = ressalva nomeada; **na B, divergência no corpo do assento = BLOQUEADO** (o objeto sob julgamento tem de ser o que roda).
