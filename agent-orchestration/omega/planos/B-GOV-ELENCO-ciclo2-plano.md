...
| A-16 | **SUPERADO pela EMENDA 2** (o 71 era do parser do ciclo 1; medido agora: **34**).  falsificação do conserto CRLF: cópia do script com `normalizado = texto` → **1 ou mais BLOQUEIA falsos** (71 no ciclo 1); restaurado → 0 | conserto não load-bearing (0 falsos sem ele) |
| A-17 | **SUPERADO pela EMENDA 2** (vira `A-17a` + `A-17b`; a forma antiga pressupunha UM mecanismo de limpeza e o desenho tem DOIS que se sobrepõem).  falsificação do conserto de cerca: cópia do script sem a remoção de cercas → voltam os **6** falsos do `skill-creator`; restaurado → 0 | idem |
...
| A-25 | **EMENDADO pela EMENDA 2** (o `sem "assento"` era proxy; passa a testar a substância: a `description` não pode creditar o assento como entregue, e deve dizer que ele vai para a fatia B).  history: entrada `B-GOV-ELENCO` com `blocks_completed: 162`, `pr/merge_commit/approved_head: null` na autoria, `description` **sem** "5 de 12", **sem** "6,6 KB", **sem** "assento"; contagens de teste CARREGADAS com nota §C3.3 | grep positivo em qualquer um dos três |
...
4. **Corpo carregado × corpo julgado (lacuna do ciclo 1):** o briefing declara **como** cada identidade é carregada (subagente nomeado do `.claude/agents/` do cwd da sessão, ou corpo colado do blob do head) e o inspetor confere, por identidade usada, comparação **EOL-neutra** — `git -C <wt> show <head>:.claude/agents/<x>.md` contra o arquivo carregado, ambos com `tr -d '\r'` antes do hash, ou `git -c core.autocrlf=false show`. **NUNCA `md5sum` cru dos dois lados** (ressalva R3 do inspetor da fatia A): a árvore de trabalho está sob `core.autocrlf=true` e chega ao disco com CRLF enquanto o blob tem LF — o hash cru **fabrica divergência** e bloquearia a fatia B por artefato de quebra de linha. É a lição do §C7.1-ter(c), que já custou uma pendência ALTA fechada por não-reprodução. Divergência = ressalva nomeada; **na B, divergência no corpo do assento = BLOQUEADO** (o objeto sob julgamento tem de ser o que roda).
