# [opção B, 2026-09-13 — cópia ajustada pelo aplicador: 56 itens; rótulos "novo" contra o HEAD; regra do §8.7 inalterada]
...
print('§4.1: itens =', len(items), '| números 1..56 completos:', sorted(items) == list(range(1, 57)))
...
for (tok, cl), (marcado, novo) in sorted(nao.items(), key=lambda x: (x[0][1], x[0][0])):
    print(f'  [{cl}] {tok}' + ('  — marcado "novo"' if marcado else '') + ('  — ESCRITO NA APLICAÇÃO B' if novo else '  — já estava no HEAD'))
print('== tokens que só resolvem por NOME DE ARQUIVO (sem diretório):')
for tok, novo in sorted(basen.items()):
    print(f'  {tok}' + ('  — ESCRITO NA APLICAÇÃO B' if novo else '  — já estava no HEAD'))
