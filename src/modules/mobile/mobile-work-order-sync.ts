...
async function processAction(
  ...
  try {
    ...
    // Ω3F-7a — o app PREENCHE a quilometragem (km) da OS pela fila offline. Exige work_orders:status (o
    // técnico de campo tem :status, NÃO :update — a base é quem corrige, via PATCH /:id/mileage). source=
    // "app". A idempotência da fila (dedup por client_action_id) já é do harness de sync (mais acima).
