...
-- PRE-CONDICAO DE PAPEL (C1-F1): rode com um papel SUPERUSUARIO ou BYPASSRLS. Sob FORCE RLS, qualquer outro papel
-- enxergaria ZERO linhas e o censo mentiria "0 grupos": o bloco abaixo RECUSA (42501 -> "censo CEGO") em vez de contar cego.
...
DO $censo$
DECLARE grupos bigint; amostra text; papel text := current_user;
BEGIN
  -- VISIBILIDADE FAIL-CLOSED (C1-F1): sob FORCE RLS, um papel sem superusuario/BYPASSRLS enxergaria um universo
  -- VAZIO e o censo mentiria "0 grupos". Com row_security = off o Postgres RECUSA (42501) toda consulta a que uma
  -- politica se aplicaria, em vez de filtrar: ou o papel enxerga TODAS as linhas, ou a migracao para AQUI.
  ...
  EXCEPTION WHEN insufficient_privilege THEN
    RAISE EXCEPTION 'stock_movements: censo CEGO sob o papel "%" (FORCE RLS sem superusuario/BYPASSRLS): este papel NAO enxerga todas as linhas e o censo NAO pode contar; NADA foi mutado. Rode esta migracao e scripts/inventory-duplicates-census.sql com um papel superusuario ou BYPASSRLS (a URL de superusuario do cluster, ou `ALTER ROLE <papel_da_migracao> BYPASSRLS`), consulte P-O6R-B04-CENSO-DUPLICATAS-STAGING-PROD e, depois, `prisma migrate resolve --rolled-back 20260873000000_add_stock_movements_unique_backstops`. Postgres: %', papel, SQLERRM
