...
// Ω3F-3a — rotas /api/v1/work-orders/:workOrderId/financial-items: RBAC (work_order_financials:*),
// congelamento via API, total agregado no DTO, idempotência 409, delete lógico 204 e isolamento.

test("POST financial-items congela o preço da Tarifa publicada e retorna 201 (DTO sem tenant_id)", async () => {
  await withFinancialApi(async ({ baseUrl, seed, seedTariff, createWorkOrder }) => {
    ...
    const created = await requestJson(baseUrl, `/api/v1/work-orders/${workOrderId}/financial-items`, {
      method: "POST",
    ...
    assert.equal(created.body.data.workOrderId, workOrderId);
...
test("GET financial-items lista com totalAmount AGREGADO no DTO (o front nunca soma)", async () => {
  await withFinancialApi(async ({ baseUrl, seed, createWorkOrder }) => {
    const workOrderId = await createWorkOrder(seed.tenantA, seed.managerA);
    ...
    const list = await requestJson(baseUrl, `/api/v1/work-orders/${workOrderId}/financial-items`, {
      headers: authHeaders(seed.tenantA, seed.managerA, "manager"),
    });
...
test("PATCH quantity recomputa o total do preço JÁ congelado (200)", async () => {
  await withFinancialApi(async ({ baseUrl, seed, createWorkOrder }) => {
    ...
    const patched = await requestJson(baseUrl, `/api/v1/work-orders/${workOrderId}/financial-items/${created.body.data.id}`, {
      method: "PATCH",
      headers: authHeaders(seed.tenantA, seed.managerA, "manager"),
      body: { quantity: 4 },
    });
...
test("DELETE lógico → 204; item some da lista e do total agregado", async () => {
  await withFinancialApi(async ({ baseUrl, seed, createWorkOrder }) => {
    ...
    const removed = await requestJson(baseUrl, `/api/v1/work-orders/${workOrderId}/financial-items/${created.body.data.id}`, {
      method: "DELETE",
      headers: authHeaders(seed.tenantA, seed.managerA, "manager"),
    });
...
test("409 idempotência: replay do mesmo client_action_id via rota → duplicate_financial_item", async () => {
  await withFinancialApi(async ({ baseUrl, seed, createWorkOrder }) => {
    const workOrderId = await createWorkOrder(seed.tenantA, seed.managerA);
    const body = { source: "manual", description: "Pedágio", unit_amount: 10, client_action_id: "act-rota-1" };
    const first = await requestJson(baseUrl, `/api/v1/work-orders/${workOrderId}/financial-items`, {
      method: "POST",
      headers: authHeaders(seed.tenantA, seed.managerA, "manager"),
      body,
    });
    const replay = await requestJson(baseUrl, `/api/v1/work-orders/${workOrderId}/financial-items`, {
      method: "POST",
      headers: authHeaders(seed.tenantA, seed.managerA, "manager"),
      body,
    });
    ...
    assert.equal(replay.body.error.reason, "duplicate_financial_item");
  });
});

test("[isolamento] OS de outra organização → 404 (POST e GET); nada vaza", async () => {
  await withFinancialApi(async ({ baseUrl, seed, createWorkOrder }) => {
    const workOrderId = await createWorkOrder(seed.tenantA, seed.managerA);
    const crossCreate = await requestJson(baseUrl, `/api/v1/work-orders/${workOrderId}/financial-items`, {
      method: "POST",
      headers: authHeaders(seed.tenantB, seed.managerB, "manager"),
      body: { source: "manual", description: "x", unit_amount: 10 },
    });
    const crossList = await requestJson(baseUrl, `/api/v1/work-orders/${workOrderId}/financial-items`, {
      headers: authHeaders(seed.tenantB, seed.managerB, "manager"),
    });
    assert.equal(crossCreate.status, 404);
    assert.equal(crossCreate.body.error.reason, "work_order_not_found");
    assert.equal(crossList.status, 404);
  });
});

test("[RBAC] POST sem work_order_financials:create (viewer) → 403; sem headers → 403", async () => {
  await withFinancialApi(async ({ baseUrl, seed, createWorkOrder }) => {
    const workOrderId = await createWorkOrder(seed.tenantA, seed.managerA);
    const asViewer = await requestJson(baseUrl, `/api/v1/work-orders/${workOrderId}/financial-items`, {
      method: "POST",
      headers: authHeaders(seed.tenantA, seed.viewerA, "viewer"),
      body: { source: "manual", description: "x", unit_amount: 10 },
    });
    const anon = await requestJson(baseUrl, `/api/v1/work-orders/${workOrderId}/financial-items`, {
      method: "POST",
      body: { source: "manual", description: "x", unit_amount: 10 },
    });
...
test("[RBAC] viewer LÊ a lista (work_order_financials:read) → 200; operator CRIA → 201", async () => {
  await withFinancialApi(async ({ baseUrl, seed, createWorkOrder }) => {
    const workOrderId = await createWorkOrder(seed.tenantA, seed.managerA);
    const read = await requestJson(baseUrl, `/api/v1/work-orders/${workOrderId}/financial-items`, {
      headers: authHeaders(seed.tenantA, seed.viewerA, "viewer"),
    });
...
// ---------- harness (espelho de tests/service-quotes-routes.test.ts) ----------
...
async function withFinancialApi(callback: (context: FinancialApiContext) => Promise<void>): Promise<void> {
  ...
  const createWorkOrder = async (tenant: Tenant, user: User): Promise<string> => {
    const created = await requestJson(baseUrl, "/api/v1/work-orders", {
      ...
      body: { title: `OS Financeiro ${randomUUID()}` },
    });
    assert.equal(created.status, 201);
  ...
  try {
    await callback({ baseUrl, seed, seedTariff, createWorkOrder });
  } finally {
...
function seedCoreSaas(service: {
  ...
  const tenantA = service.createTenant({ name: "Tenant Financials A", modules: ["dashboard", "work_orders"] });
  const tenantB = service.createTenant({ name: "Tenant Financials B", modules: ["dashboard", "work_orders"] });
  const managerA = service.createUser({ tenantId: tenantA.id, name: "Manager A", email: "financials-manager-a@example.com", roles: ["manager"] });
  const managerB = service.createUser({ tenantId: tenantB.id, name: "Manager B", email: "financials-manager-b@example.com", roles: ["manager"] });
  const operatorA = service.createUser({ tenantId: tenantA.id, name: "Operator A", email: "financials-operator-a@example.com", roles: ["operator"] });
  const viewerA = service.createUser({ tenantId: tenantA.id, name: "Viewer A", email: "financials-viewer-a@example.com", roles: ["viewer"] });
