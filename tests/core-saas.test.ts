...
test("mantem roles padrao coerentes com o catalogo RBAC", () => {
  ...
  // Ω5P PR-02 (D-Ω5P-JUR-03) — Perfis normativos: `jurisdiction:read` = MESMO conjunto de `yard:read`/`branches:read`
  // (manager+operator+field_dispatcher+technician+field_technician+viewer+auditor+admins); `jurisdiction:create`/
  // `jurisdiction:update` = SÓ manager+admins. `jurisdiction:manage` NÃO existe (sem guard/rota = permissão morta).
  for (const role of [
    "manager",
    "operator",
    "field_dispatcher",
    ...
    assert.equal(ROLE_PERMISSIONS[role].includes("jurisdiction:read"), true);
    assert.equal(ROLE_PERMISSIONS[role].includes("yard:read"), true);
  }
  // Espelha yard/branches: finance/inventory/support NÃO leem Perfis normativos.
  for (const role of ["finance", "inventory", "support"] as const) {
    assert.equal(ROLE_PERMISSIONS[role].includes("jurisdiction:read"), false);
  }
  // create/update = gestão+admins (manager + tenant_admin/super_admin/platform_admin), NUNCA os demais.
  for (const role of ["manager", "tenant_admin", "super_admin", "platform_admin"] as const) {
    assert.equal(ROLE_PERMISSIONS[role].includes("jurisdiction:create"), true);
    assert.equal(ROLE_PERMISSIONS[role].includes("jurisdiction:update"), true);
  }
  for (const role of [
    "operator",
    "field_dispatcher",
    ...
    assert.equal(ROLE_PERMISSIONS[role].includes("jurisdiction:create"), false);
    assert.equal(ROLE_PERMISSIONS[role].includes("jurisdiction:update"), false);
  }
  ...
  // Ω5P PR-09 — impound:notify (EMISSÃO/DISPENSA de notificações legais I6) espelha a MESMA distribuição
  // (gestão+admins). A DUE-marking é do SISTEMA (job), sem ponta HTTP.
  for (const role of ["manager", "tenant_admin", "super_admin", "platform_admin"] as const) {
    ...
    assert.equal(ROLE_PERMISSIONS[role].includes("impound:notify"), true);
  }
  for (const role of [
    "operator",
    "field_dispatcher",
    ...
    assert.equal(ROLE_PERMISSIONS[role].includes("impound:notify"), false);
  }
  ...
  // Ω5P PR-10a (D-Ω5P-REL / D-Ω5P-07 / D-Ω5P-12) — Liberação I5: `charging:settle` (QUITAÇÃO — ATO DE DINHEIRO,
  // = charging:create), `release:process` (dossiê recipient+checks) e `release:approve` (ATO DA AUTORIDADE,
  // stand-in tenant_admin+manager) = SÓ manager+admins (gestão+admins). Os SALTOS da FSM reusam impound:transition.
  for (const role of ["manager", "tenant_admin", "super_admin", "platform_admin"] as const) {
    assert.equal(ROLE_PERMISSIONS[role].includes("charging:settle"), true);
    assert.equal(ROLE_PERMISSIONS[role].includes("release:process"), true);
    assert.equal(ROLE_PERMISSIONS[role].includes("release:approve"), true);
  }
  for (const role of [
    "operator",
    "field_dispatcher",
    ...
    assert.equal(ROLE_PERMISSIONS[role].includes("charging:settle"), false);
    assert.equal(ROLE_PERMISSIONS[role].includes("release:process"), false);
    assert.equal(ROLE_PERMISSIONS[role].includes("release:approve"), false);
  }
