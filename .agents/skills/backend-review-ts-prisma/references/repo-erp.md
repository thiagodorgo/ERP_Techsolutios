...
- `$queryRaw`/`$executeRaw` com template tag parametrizado: 25 arquivos em `src/` usam. **`$queryRawUnsafe` tem exatamente uma ocorrência legítima** (`src/routes/health.routes.ts`, literal `SELECT 1`). Qualquer segunda ocorrência com entrada externa é achado **Crítico**.
