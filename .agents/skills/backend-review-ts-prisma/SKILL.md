...
description: "Revisão técnica de backend Node.js + TypeScript + Prisma + PostgreSQL no ERP Techsolutions. Usar ao analisar PR, diff, branch, commit, arquivo ou módulo de src/**, tests/** e prisma/** para achar bug, quebra de autorização/tenant, corrida, consulta ineficiente, falha de integridade e lacuna de teste. É também o método do ASSENTO PERMANENTE da junta (D-CADEIRA-PERMANENTE-JUNTA): usar em TODA junta, depois dos votos de mérito e antes do merge, para medir se a aprovação foi ganha e se o veto foi legítimo. NÃO usar para decidir modelagem multi-tenant nova (essa é a saas-multi-tenant) nem para front/Flutter (ts-frontend-full)."
---
...
## Os dois modos desta skill

**Modo revisão** (o default) — revisa **código**: um PR, um diff, um módulo. É o resto deste arquivo.

**Modo cadeira permanente** — revisa **o julgamento**. Esta skill ocupa o **assento permanente da junta**
(`D-CADEIRA-PERMANENTE-JUNTA`, 2026-09-07), encarnado no agente
[`cadeira-permanente-backend-review`](../../agents/cadeira-permanente-backend-review.md). É a única cadeira
não-descartável: entra em **toda** junta, depois dos votos de mérito e antes do merge, e mede se a
**aprovação foi ganha** e se o **veto foi legítimo**. Nesse modo valem três inversões em relação ao default:

| | Modo revisão | Modo cadeira permanente |
|---|---|---|
| Objeto | o código do bloco | os **votos** das cadeiras de mérito |
| Achado de produto | é o produto do trabalho | **não é seu** — vira pendência nomeada, nunca voto |
| Saída | achados por gravidade | `HOMOLOGADO` · `HOMOLOGADO COM RESSALVA` · `ANULADO POR APROVAÇÃO NÃO GANHA` · `ANULADO POR VETO ILEGÍTIMO` |

O mandato completo do assento — o que medir, a fronteira que impede de virar superjurado, o formato do
parecer — está no arquivo do agente. **Ocupando o assento, aquele arquivo manda; esta skill é o método.**
