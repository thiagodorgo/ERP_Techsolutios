...
B-108 adiciona `EvidenceStorageProvider`, provider local protegido para dev/test, `EvidenceScanner` com registro unico e fake em testes, auditoria segura em memoria para `evidence.upload.accepted`, `evidence.upload.rejected`, `evidence.upload.scan_failed` e `evidence.upload.stored`. Falha de scanner retorna erro seguro `evidence_scan_failed`; arquivo rejeitado retorna `evidence_rejected`. Lacunas remanescentes: presigned URL/objeto externo, persistencia duravel DB/Redis do receipt, antivirus real, retencao definitiva e download protegido final. **B-O6R-07b (Omega6R-SEC-004) atualiza esta linha:** o default do scanner deixou de ser Noop-em-todo-lugar e passa a ser derivado do `NODE_ENV` pelo registro unico (`src/modules/evidence/evidence-scanner.factory.ts`) — `production` (staging incluso) resolve `unavailable` (todo upload responde 503, nada e gravado) e dev/test resolve `noop`; `EVIDENCE_SCANNER=noop` com `NODE_ENV=production` e RECUSADO no boot. Alem disso, o tipo do arquivo passa a sair dos BYTES (sniff de assinatura in-house) tanto no upload quanto no download, e o egresso serve `attachment` + `nosniff`. O antivirus real segue como lacuna, agora com dono nomeado: `P-O6R-B07B-SCANNER-AV-REAL`. B-098E permanece responsavel apenas pelos metadados/idempotencia do manifesto.
