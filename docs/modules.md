...
- `field_operator_location` e a fundacao backend tenant-scoped para localizacao de operadores em campo. Ela cria `field_operator_locations`, aplica RBAC/RLS, recebe coordenadas via `/api/v1/mobile/field-locations` e expõe ultimas posicoes/historico para o Mapa Operacional, sem implementar Google Maps real, Flutter, roteirizacao, Work Orders completas ou despacho completo.
- `field_operations` e o modulo habilitavel por tenant para mapa operacional, operadores em campo e despachos futuros. Nesta branch ele sustenta a rota `/operations/map`, que entrega UI inicial com KPIs, filtros, mapa placeholder, marcadores proporcionais por lat/lng, lista, detalhe e fallback/mock seguro.
- `work_orders` e a fundacao backend tenant-scoped para Ordens de Servico. Ela cria `work_orders`, `work_order_events` e `work_order_assignments`, aplica RBAC/RLS, expõe CRUD/status/assign/timeline por API e prepara vinculos futuros com checklists, Mapa Operacional, evidencias, despacho e comissoes, sem implementar UI completa, despacho avancado, roteirizacao, pagamentos ou estoque.
