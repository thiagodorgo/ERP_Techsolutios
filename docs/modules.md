...
- `field_dispatch` e a fundacao backend tenant-scoped para despacho operacional com UI web inicial. Ela cria `field_dispatches` e `field_dispatch_events`, vincula despacho a `work_order_id` e `operator_user_id`, aplica RBAC/RLS, valida OS e operador no mesmo tenant, registra eventos/auditoria best-effort, expoe endpoints `/api/v1/operations/dispatches` e agora sustenta `/operations/dispatches` no frontend para listagem, KPIs, filtros, criacao simples, detalhe, status/cancelamento e reatribuicao. Roteirizacao, algoritmo de rota, WebSocket, Google Maps real e app Flutter permanecem fora desta branch.
- `field_operations` e o modulo habilitavel por tenant para mapa operacional, operadores em campo e despachos. Nesta branch ele sustenta as rotas `/operations/map` e `/operations/dispatches` no frontend, exibe despacho vinculado no mapa quando `field_dispatch:read` esta presente, cria navegacao contextual para `/operations/dispatches?workOrderId=...&operatorUserId=...` quando `field_dispatch:create` esta presente e permite alterar status, cancelar ou reatribuir despachos diretamente no detalhe do operador conforme `field_dispatch:update`, `field_dispatch:cancel` e `field_dispatch:reassign`. O mapa usa Google Maps real quando `VITE_GOOGLE_MAPS_API_KEY` esta configurada, com fallback automatico para mapa placeholder. O polling frontend de 30 s atualiza localizacoes, OS e despachos sem reload manual, preserva selecao do operador e pode ser pausado/retomado; WebSocket e roteirizacao avancada permanecem fora do escopo.
- `frontend_route_splitting` e a otimizacao de performance aplicada em `App.tsx` via `React.lazy` + `Suspense`. Reduz o chunk JS principal de 512 kB para 389 kB eliminando o warning Vite de chunk acima de 500 kB. Rotas pesadas (operations map, dispatches, work-orders, platform cloud-billing, platform tenants) sao carregadas sob demanda; rotas de critical path (login, dashboard, checklists) permanecem eager. Guards e permissoes preservados.
- `frontend_manual_vendor_chunks` e a otimizacao de cache de longo prazo aplicada em `vite.config.ts` via `build.rollupOptions.output.manualChunks`. Separa `react`/`react-dom`/`react-router-dom`/`scheduler` em `vendor-react` (232 kB) e `lucide-react` em `vendor-icons` (38 kB). Chunk principal de app reduz de 389 kB para 125 kB (-67%). Vendors ficam em cache do browser entre deploys; apenas o chunk de app precisa ser re-baixado em atualizacoes de produto. Route splitting da PR #61 preservado.
...
- `expense_management` e o modulo oficial de Gestao de Despesas. Gestor de Despezas (GD) fica como alias historico e Prestação de Contas como documento gerado. O modulo deve ser tenant-scoped, habilitado por plano/tenant/papel/RBAC, preparado para operacao Flutter local-first, sync idempotente, recibos, OCR/PDF futuros, aprovacao manager/finance, auditoria e eventos assincronos. Pagamento real, fiscal, contabil, conciliacao bancaria, cartao corporativo, backend completo e acoplamento direto com comissoes ficam fora da fundacao inicial.
- `expense_management_backend_foundation` adiciona a fundacao backend real do modulo: schema tenant-scoped com RLS, permissoes RBAC `expense_*`, rotas MVP `/api/v1/expense-*` e `/api/v1/mobile/sync/expense-actions`, repositorio em memoria para testes, repositorio Prisma lazy/RLS para runtime e sync idempotente por `tenant_id` + `client_action_id`. OCR/upload/PDF/pagamento/fiscal/UI web/approval avancado/integracao direta com comissoes seguem fora desta fase.
- `mobile_backend_contract_readiness` e a consolidacao B-098 dos contratos backend para o MVP mobile. Ela formaliza `GET /api/v1/mobile/bootstrap` como bootstrap minimo, documenta endpoints reais de auth, OS, checklists, despesas, notificacoes, localizacao e sync parcial, e explicita lacunas de sync de OS/checklist/inventario e evidencia generica. Nao altera Flutter, nao implementa sync completo e nao substitui as fases futuras B-098A..B-098F.
