...
- `field_ops_sse_transport` é o transporte SSE backend tenant-scoped para eventos de operações de campo. Cria `FieldOpsBroadcaster` (in-memory, EventEmitter, canais por tenant) em `src/infra/broadcaster/field-ops.broadcaster.ts`; atualiza o handler `field-ops-event-fanout.jobs.ts` para publicar no broadcaster; adiciona endpoint `GET /api/v1/operations/events/stream` em `src/modules/field-ops/field-ops-sse.routes.ts`. O endpoint exige ao menos uma das permissões `field_location:read`, `field_dispatch:read` ou `work_orders:read`; filtra cada evento pela permissão específica antes de entregar; strip de coordenadas (latitude/longitude/lat/lng) no payload de `field_location.updated`; heartbeat de 30 s como comentário SSE; cleanup de subscriber no close do cliente. Isolamento cross-tenant garantido por canal `tenant:{tenantId}` no broadcaster. Nenhum WebSocket; nenhuma alteração no frontend; nenhuma migration; polling de 30 s preservado intocado.
