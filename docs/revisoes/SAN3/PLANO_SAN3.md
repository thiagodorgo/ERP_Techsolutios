...
| `B-SAN3-07` · `fix/seed-demo-nome-de-negocio` | 47 | nome técnico no seed | `prisma/seed.ts` (**autorizado** só para o nome); **ampliação nominal declarada** (`B-SAN3-00`, dívida 3 do parecer do porteiro pós-merge do #390): o **mesmo** `prisma/seed.ts` fica autorizado **também** para semear os cinco papéis legados da `P-SAN3-04A-SEED-PAPEIS-LEGADOS` (`finance`, `inventory`, `operator`, `field_technician`, `support`) — mudança **aditiva**: nenhum papel existente é removido e nenhuma concessão é retirada | re-seed numa base descartável e `SELECT name FROM tenants` → nenhum contém "Tenant" (medido no banco, não no fonte) | backend | maioria | `SAN3-04a` (mesmo arquivo) | P |
