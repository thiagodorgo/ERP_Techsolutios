...
2. **O registro errava o status de 52 das 231 entradas (22,5%)** — 31 fechadas no código que o texto dava como
   ...
   54. Índice (gerador, depois da opção B): 370 cabeçalhos (359 IDs); FECHADAS 72 → 103; ABERTAS 267.
3. **O gate da versão vendável tem 56 bloqueantes, fechados por 37 blocos, e 6 atos que só o dono pratica**
...
4. **Não cabe em 48 horas.** O melhor caso — 4 frentes em paralelo, cada bloco no tempo do mais rápido já
   ...
   gate só é declarado quando os 56 bloqueantes fecharem **e** os 6 atos estiverem feitos.
...
| 7 | `Ω6R-ARQ-004` (em `P-O6R-B09`) — despacho e evento obrigatório em transações separadas, sem chave idempotente | 5 | `src/modules/field-dispatch/field-dispatch.service.ts:138-161` (O6R: risco "perda/corrupção") | `B-O6R-09` | ✓ |
...
| 16 | `P-WEB-GATE-MODULO-INCOMPLETO` + `P-NAV-MODULOS-NAO-RESOLVIDOS-LIBERA-MENU` — **40** itens do menu fora do gate de módulo: os 27 do método MVP menos registro **mais** as 13 entradas de organização registradas sem `requiredModules` (`/patios/*` ×6, `/telemetria/*` ×5, `/controle/notificacoes`, `/operations/quotes`), liberadas por `hasModule`; nenhuma recusa por módulo achada no backend; sem chave de módulo para Pátios, Telemetria e Frota; e a lista de módulos não resolvida libera todo item de módulo no menu | 3 | AUSENTES (backend medido por padrões, não exaustivo); `src/modules/navigation/navigation.service.ts:114-115`; `src/modules/platform/platform-modules.service.ts` (C2-02; ciclo 2: C2c2-02 e C2c2-05) — **condicional: venda por plano/módulo** (default: sim) | `B-SAN3-18` | |
...
| 56 | `P-RBAC-MATRIZ-X-CATALOGO-QUATRO-CELULAS` — quatro células de ação da matriz sem permissão no catálogo (inclui o `finance` sem `work_orders:read`) | 3 | `src/modules/core-saas/permissions/catalog.ts:730-894` e `RBAC_MATRIX.md:37,44-46` (junta do ciclo 2, lacuna da C1; conferido pelo orquestrador) | `B-SAN3-04a` | ✓ |
...
| 55 | `P-WEB-CONCILIACAO-SEM-TELA` — a conciliação é prometida (notas `mvp_*`, `API_CONTRACTS.md:435`) e a web não concilia: sem extrato nem ação | 7, 4 | rota em `src/modules/financial-entries/financial-entry.routes.ts:85`; 0 chamada a `/reconcile` e a `GET /financial-entries` em `frontend/src` (junta do ciclo 2, C1-A1; medido de novo na aplicação da opção B) | `B-SAN3-12` (ampliado) | |
...
fechamento das duas, provado por mutação, é o teste (g) do `B-SAN3-10` (item 54): no `roadmap`, bloco com
`merge_commit` no history não fica `a_fazer`, e `roadmap.as_of` ≥ último merge; o `recent` contém o último merge. A causa comum — nenhum PR de entrega é obrigado a alimentar essas duas seções — fica com
...
| `P-SAN3-INDICE-ITEM-DO-GATE-SO-PELA-HOSPEDEIRA` | não viola, no código do produto, nenhum dos 13 critérios: é o índice de registro, e o §1 declara que não usa a severidade dele | o gate é lido no §4.1, não no índice (§1); dono `B-REG-GERADOR`, fila pós-gate |
...
| `B-O6R-12` · `fix/jurisdiction-profile-versioning` | 24 | o motor de diárias lê o perfil vivo | `src/modules/jurisdiction/**`, `src/modules/charging/**` (leitura do regime), `src/modules/auction/auction.eligibility.ts`, `src/modules/impound/impound-prisma.repository.ts` (o processo nasce ali, l.69 e 114 — carimbo do regime na entrada); **autorizado** `prisma/schema.prisma` + `prisma/migrations/**` só para o snapshot aditivo do regime no processo | editar o perfil com processo vivo → diárias do processo inalteradas; auditoria campo a campo; o aceite provisório do `PLANO_O6R` é ratificado pela junta antes do código | backend | unanimidade + crítico | `SAN3-20` (trava de `prisma/`, §6), `SAN3-11` (`src/modules/impound/impound-prisma.repository.ts`, §6) | M |
| `B-SAN3-18` · `fix/gate-de-modulo-no-backend` | 16 | o gate de módulo vive só no menu | `src/modules/navigation/navigation.registry.ts`, `src/modules/navigation/navigation.service.ts` (o `hasModule` sem módulo deixa de liberar), `src/modules/platform/platform-modules.service.ts` (chaves de Pátios, Telemetria e Frota), `src/app.ts` (a recusa entra no **ponto de montagem** das rotas, por um middleware novo em `src/modules/navigation/`, não nos 12 routers), `frontend/src/layouts/appSidebarNav.ts`, `prisma/seed.ts` (**autorizado** só para as chaves de módulo novas em `DEMO_TENANT_MODULES` — CE-1); o provisionamento das chaves novas para as organizações existentes é script aditivo do bloco em `scripts/` (novo), com contagem antes e depois (C2-02). A lista dos 40 caminhos: os 27 da **diferença medida** entre `MVP_NAV_PATHS` (51) e o registro (36), gerada por script no plano do bloco, mais as 13 entradas de organização do registro sem `requiredModules` | organização sem o módulo → rota responde 403 nomeado (hoje 200) e o item some; guard **fail-closed**: toda entrada do registro tem `requiredModules` **ou** está numa lista fechada de núcleo, e o guard falha para entrada sem módulo fora da lista; todo caminho do menu está no registro + CE-G1, CE-1 (§5.6) | backend, frontend | unanimidade + `coordenador-de-acessos` | `SAN3-04a`, `SAN3-06a` (mesmo arquivo de menu), `SAN3-07` (`prisma/seed.ts`, §6) | G |
...
| `B-SAN3-01b` · `fix/web-guarda-por-alcance-e-estado-da-pagina` | 4 (as **guardas** da propriedade que o `SAN3-01` fechou) + as 4 pendências `P-SAN3-01B-PAGINA-NAO-AMARRADA-AO-ESTADO`, `P-SAN3-01B-GUARD-ALCANCE-MENOR-QUE-AS-RAIZES`, `P-SAN3-01B-VIGIA-TEXTUAL-DA-FIACAO` e `P-SAN3-01-NOVA-OS-SEM-GATE-NO-BOTAO` | o código de hoje **não** fabrica dado — o que falta é a garantia de que a próxima linha não reintroduza a fabricação sem nada ficar vermelho: a decisão da página não está amarrada ao estado do reducer, e o guard do mock tem alcance menor que as próprias raízes | `frontend/src/modules/work-orders/pages/WorkOrdersPage.tsx` (a página real e o gate do botão "Nova OS"), `frontend/src/modules/work-orders/useWorkOrders.ts` (o vigia `[W1]` passa a medir comportamento), `frontend/src/modules/work-orders/repository.ts`, `frontend/src/modules/registry/service-quotes/useServiceQuoteReferences.ts` e `frontend/src/modules/operations/dispatches/dispatches.service.ts` (**só** o texto de fechamento dos cabeçalhos, que hoje afirma alcance que a mutação desmente), `frontend/tests/work-orders-honest-errors.test.tsx` (o guard `[G1]` e os vigias), `frontend/package.json` (**só** a lista do `test:smoke`, se o bloco criar arquivo de teste novo); **sem backend, sem migração** | as quatro mutações da cadeira C4 ficam **vermelhas**: `N-PG-PAINEL` (trocar `status={status}` por `status="empty"` na página → hoje 67/67 verdes, `tsc` e smoke verdes), `N-BARREL2` (barrel de N níveis → o mock chega à tela em modo real), `N-LITERAL` (entidade fabricada inline dentro das raízes) e `N-W1TXT` (envolver o `setState` mantendo as asserções textuais); e o botão "Nova OS" só aparece com `work_orders:create` | frontend | unanimidade (perda de dado — §5 acima) + `cognicao-visual` + `coordenador-de-acessos` (gate do botão) | `SAN3-01` ✓ (#387 — mesmos arquivos) | M |
| `B-SAN3-00b` · `chore/ignore-global-por-nome` | a pendência `P-SAN3-00-IGNORE-GLOBAL-POR-NOME-DENTRO-DOS-REINCLUIDOS` | o `B-SAN3-00` desfez o ignore **do diretório** (`.claude/`, `.agents/`) mas **não** os ignores **por NOME**, que continuam casando lá dentro: 86 de 88 sondas escondidas, geradas da fonte (22 padrões sem barra × 4 diretórios). Arquivo novo com um desses nomes nasce invisível, que é a classe que o `B-SAN3-00` existiu para fechar — fechou o diretório, não o nome | `.gitignore` (**só** as exceções nominais), `tests/` se o bloco criar guard | a exceção nominal cobre os nomes que importam dentro de `agents/` e `skills/`, provada por `git check-ignore -q --no-index` nas duas pontas, com a classe **gerada da fonte** e não por lista escrita à mão; e **0** arquivo hoje rastreado passa a ser ignorado | registro | maioria + `agente-secops` | `SAN3-00` ✓ | P |
| `B-GOV-ATA-CABECALHO` · `docs/ata-cabecalho-e-approved-head` | o cabeçalho da ata vira contrato: título nomeia o PR, convenção multi-ciclo, e a linha estrutural `- **approved_head:** \`sha\`` que a junta escreve ao aprovar | medido no `B-GOV-MANDATO` ciclo 2: **`linha_estrutural_approved_head` = 0/107 atas**, logo o estado `LIDO` do `mandato-refs.sh` é **inalcançável no corpus de hoje**; só 10/107 têm veredito estrutural, em **3 grafias**, e uma traz `APROVADO` e `REPROVADO` na mesma linha — ler veredito seria enumerar formas, que é a classe que o bloco fechou | `agent-orchestration/omega/juntas/TEMPLATE-J-ata.md`, retrofit das **4** atas APROVADAS que já têm linha de Objeto, `docs/` de convenção; **`scripts/mandato-*.sh` e `tests/mandato-*` PROIBIDOS** (são do bloco anterior) | o template emite a linha; as 4 retrofitadas saem `LIDO` por execução; ata reprovada segue `NÃO DETERMINÁVEL`; e **nenhuma das 103 restantes passa a ser lida por engano** | registro | maioria | `B-GOV-MANDATO` ciclo 2 ✓ | P |
| `B-GOV-MANDATO-2` · `chore/mandato-refs-fronteiras` | as fronteiras que o ciclo 2 mediu e deixou nomeadas em `P-GOV-MANDATO-2-FRONTEIRAS` | a proveniência do `mandato-refs.sh` é um **retrato, não um histórico** — o pré-voo recusou o head de partida do próprio dev quando o ramo andou, dizendo "pode ser SHA VELHO"; rota de API dentro de um comando cai na checagem 6; e **um md5 tem 32 hex e cai na faixa de SHA** da checagem 4 | `scripts/mandato-*.sh`, `tests/mandato-*.test.ts` | cada fronteira com caso de encerramento executado e vermelho-controle, sem reabrir o que o ciclo 2 fechou | registro | maioria | `B-GOV-MANDATO` ciclo 2 ✓ | M |
| `B-SAN3-12` · `feat/web-financeiro-baixa-e-contas` | 28, 55 | a web emite título e não liquida nem concilia | `frontend/src/modules/finance/**` sobre `POST /financial-titles/:id/pay`, as rotas de conta, o extrato (`GET /financial-entries`) e a conciliação (`PATCH /financial-entries/:financialEntryId/reconcile`) — CE-4, `frontend/src/App.tsx` (rota), `frontend/src/layouts/appSidebarNav.ts` (menu) | título baixado pela web → `paid` e lançamento em caixa; saldo pela rota real; estados §7; ação só com a permissão + CE-G2, CE-4 (§5.6) | frontend | unanimidade + `cognicao-visual` + `coordenador-de-acessos` | `SAN3-04a` (mesmo arquivo de menu; as permissões do Financeiro — C1-06) | M |
...
| `B-SAN3-06a` · `fix/menu-sem-tela-ficticia` | 39 | telas de protótipo roteadas e no menu, sem dado | `frontend/src/modules/{purchase-orders,reports}/**`, `frontend/src/modules/dispatch/pages/DispatchConsolePage.tsx`, `frontend/src/layouts/appSidebarNav.ts`, `frontend/src/navigation/tenantNavigation.ts`, `frontend/src/App.tsx`; **ampliação nominal declarada** (`B-SAN3-00`, dívida 3 do parecer do porteiro pós-merge do #390): `frontend/src/modules/auth/auth.adapter.ts` — **só** o mapa estático `rolePermissions` (l.260-356), dono da `P-SAN3-04A-FRONT-PERMISSOES-POR-PAPEL-DEFASADAS`, e `docs/navigation-matrix.md`, dona da `P-SAN3-04A-NAVIGATION-MATRIX-DEFASADA` (a própria pendência já prescrevia a ampliação; aqui ela deixa de ser promessa e vira linha do §5) | nenhuma tela do menu exibe dado constante; ou a tela sai do menu e da rota (default do §10.1) | frontend | unanimidade + `coordenador-de-acessos` + `cognicao-visual` | `SAN3-24` (mesmo arquivo de menu) | M |
| `B-SAN3-07` · `fix/seed-demo-nome-de-negocio` | 47 | nome técnico no seed | `prisma/seed.ts` (**autorizado** só para o nome); **ampliação nominal declarada** (`B-SAN3-00`, dívida 3 do parecer do porteiro pós-merge do #390): o **mesmo** `prisma/seed.ts` fica autorizado **também** para semear os cinco papéis legados da `P-SAN3-04A-SEED-PAPEIS-LEGADOS` (`finance`, `inventory`, `operator`, `field_technician`, `support`) — mudança **aditiva**: nenhum papel existente é removido e nenhuma concessão é retirada | re-seed numa base descartável e `SELECT name FROM tenants` → nenhum contém "Tenant" (medido no banco, não no fonte) **e** os 5 papéis semeados existem com as concessões do catálogo | backend | **unanimidade** + `coordenador-de-acessos` (§C7.1-ter(b): com a ampliação nominal acima o bloco passa a **semear papéis globais com as concessões do catálogo**, logo toca **permissão**; era `maioria` quando a fronteira era só o nome da organização — achado **C1-A5** da junta do `B-SAN3-00`, que foi o bloco a alargar a fronteira) | `SAN3-04a` (mesmo arquivo) | P |
| `B-SAN3-08` · `fix/orcamento-destino-e-papel-financeiro` | 40, 41 | a aprovação não pergunta o destino; o Financeiro não tem as permissões de leitura do orçamento | `frontend/src/modules/registry/service-quotes/**`, `frontend/src/modules/work-orders/components/tabs/QuoteTab.tsx`; os grants chegam pelo `SAN3-04a` | orçamento com destino obrigatório → diálogo → OS criada; Financeiro cria orçamento na base com as permissões do banco + CE-G2, CE-5 (§5.6) | frontend | unanimidade + `cognicao-visual` | `SAN3-01`, `SAN3-04a` | M |
| `B-SAN3-25` · `feat/web-faturar-os` | 50 | a rota de faturamento existe e nenhuma tela a chama | `frontend/src/modules/work-orders/components/tabs/FinancialTab.tsx`, o serviço de OS em `frontend/src/modules/work-orders/**` (chamada a `POST /work-orders/:id/invoice`), `frontend/src/modules/finance/pages/InvoicesPage.tsx` | OS com itens → faturar → título criado e itens carimbados; refaturar → recusado; o delta do item 21 (título complementar) faturável pela mesma ação depois do `SAN3-02` + CE-G2, CE-3 (§5.6) | frontend | unanimidade (dinheiro) + `cognicao-visual` | `SAN3-01` (mesmo módulo), `SAN3-02` (o delta), `SAN3-24` (`frontend/src/modules/finance/**`, §6), `SAN3-04a` (o `work_orders:read` do `finance` — CE-3) | M |
...
| `B-SAN3-21` · `fix/web-acentuacao` | 49 | literais antigos sem acento | literais de texto em `frontend/src/**/*.ts` **e** `*.tsx` (só strings) | guard: lista de termos sem acento (Situacao, Nao, Configuracoes, Operacao…) em `.ts` e `.tsx` = 0 — cobre os 13 textos de `.ts` que o crítico mediu + CE-G1 (§5.6) | frontend | maioria | todos os blocos de `frontend/` (é o último a tocar texto) | P |
...
### 5.6 Condições de entrada — decisão do dono (opção B, 2026-09-13)

O dono aprovou o plano com os bloqueios da junta do ciclo 2 escritos como condição de entrada — `D-SAN3-PLANO-OPCAO-B`. Um bloco com condição só começa com o plano dele cumprindo-a, e a junta do bloco a confere por execução.

- **CE-G1 — enumeração fail-closed (geral; classe do C2c2-01, C2c2-03 e C2c2-08).** Todo guard ou censo citado num teste de encerramento do §5 declara, no plano do bloco: (a) a fonte da enumeração, **gerada por script do código real** (rotas montadas no `src/app.ts`, catálogo, registro — nunca lista curada); (b) o default do membro não previsto, que é **negar**; (c) a mutação que deixa o guard vermelho (membro novo sem classificação). Vale para todo bloco com guard, não só para os nomeados abaixo.
- **CE-G2 — papel × passo (geral; classe do C1-A2, do C1-A3 e do C1-02 do ciclo 1).** Todo passo de teste de encerramento executado por um papel tem, no plano do bloco, as permissões desse papel medidas no catálogo e a permissão exata que a rota compara. Passo cujo papel não tem a permissão só entra no teste se um bloco anterior na agenda a concede e o teste desse bloco a prova.
- **CE-1 — `B-SAN3-18` (item 16; C2c2-01, C2c2-02, C2c2-04, C2c2-05, C2c2-08).** A recusa por módulo no backend nasce de um mapa rota → módulo **gerado das montagens do `src/app.ts`**: todo router montado e todo endpoint dele classificado (módulo, ou lista **literal** de núcleo, independente do registro); rota sem classificação é **recusada**; o guard fica vermelho por mutação para (i) entrada nova do registro sem módulo, (ii) endpoint novo e (iii) router novo montado sem classificação. A contagem do item 16 é **40** — 27 da diferença `MVP_NAV_PATHS` − registro mais 13 entradas de organização sem `requiredModules` (as 11 de `/patios` e `/telemetria`, `/controle/notificacoes` e `/operations/quotes`), medida por script sobre o registro e o menu reais. A fronteira ganha `prisma/seed.ts`, **autorizado nominalmente** só para as chaves de módulo novas em `DEMO_TENANT_MODULES` (CI, e2e e demo), e o default da lista de módulos não resolvida passa a ser negar (`P-NAV-MODULOS-NAO-RESOLVIDOS-LIBERA-MENU`, que entra no item 16).
- **CE-2 — `B-O6R-07c` (itens 11 e 51; C2c2-03).** O `Ω6R-SEC-002` e o item 51 fecham por **escopo provado**, não por dono: o censo é **gerado por script** das rotas mutantes dos routers da fronteira (`src/modules/work-orders/work-order.routes.ts`, `src/modules/work-order-comments/work-order-comment.routes.ts`, `src/modules/checklists/checklist.routes.ts`) e do sync mobile, cruzadas com as permissões do `field_technician` no catálogo; **toda** rota mutante que o técnico alcança tem teste de 403 `not_assigned_to_actor` com vermelho-controle no head-base — inclusive as 5 de comentário, as 2 de geocode e as 3 de vistoria (anexo, avaria, divergência) que a C2 mediu (as 10 existem, e o técnico tem a permissão de cada uma — medido por script sobre as rotas e o catálogo); o piso "1, 2 e 10" deixa de ser critério de fechamento; um guard fica vermelho quando surge rota mutante alcançável pelo técnico sem teste de escopo. Via achada fora da fronteira continua virando pendência com dono, mas o `Ω6R-SEC-002` só fecha quando **todas** as vias do censo tiverem escopo provado.
- **CE-3 — `B-SAN3-25` (item 50; C1-A2; decisão P1).** O `finance` passa a ter `work_orders:read`, concedido pelo `B-SAN3-04a` (anterior na agenda; a matriz já dá `read`); o teste do `B-SAN3-25` fatura **como o papel `finance`, com as permissões do banco** depois do `db:provision-rbac`, pela aba da OS — além do `tenant_admin`. Dep. do `B-SAN3-25` ganha `SAN3-04a`.
- **CE-4 — `B-SAN3-12` ampliado (item 55 novo; C1-A1, C1-A7).** O extrato da conta (`GET /financial-entries`) e a ação de conciliar (`PATCH /financial-entries/:financialEntryId/reconcile`, permissão `financial_entries:update`, que o `finance` tem — `src/modules/core-saas/permissions/catalog.ts:870`) entram no escopo; teste: criar conta → emitir título → baixar pela tela → o lançamento aparece no extrato → conciliar → conciliado; conciliar de novo → a resposta que o contrato define; saldo pela rota real. O `B-SAN3-12` segue **M** no melhor caso — o planejador do bloco reavalia e, se crescer, recalcula a agenda no plano dele.
- **CE-5 — `B-SAN3-08` e `B-SAN3-04a` (itens 41 e 56; C1-A3; decisão P2).** O `B-SAN3-04a` concede ao `finance` `customers:read` e `service_catalog:read` (mais o `work_orders:read` do CE-3) e **atualiza a `RBAC_MATRIX.md` no mesmo bloco** (Clientes l.38 e Serviços l.41: `finance` de `none` para `read`) — mudança de arquivo-base autorizada pela `D-SAN3-PLANO-OPCAO-B`; o teste do `B-SAN3-04a` prova as três leituras com as permissões do banco. A fronteira do `B-SAN3-04a` ganha `RBAC_MATRIX.md` (autorizado nominalmente, só as linhas 38, 41 e as das quatro células do CE-6).
- **CE-6 — `B-SAN3-04a` (item 56 novo).** As quatro células da `P-RBAC-MATRIZ-X-CATALOGO-QUATRO-CELULAS` convergem à matriz — e onde a matriz diz "por escopo" e o backend não aplica o escopo, a permissão **não** é concedida sem ele: vira pendência nomeada com dono (fail-closed). Guard de célula da matriz × catálogo com mutação.
- **CE-7 — `B-SAN3-26` (item 53; C2c2-07).** A conferência do check-in fica no **ponto único de decisão** do servidor que as duas entradas usam (`changeStatus` → `arrived`, pelo sync e pelo `PATCH /work-orders/:id/status`), e o plano do bloco escreve o default para OS sem o dado conferido (placa ou número de série): negar, com exceção só por tipo de serviço nomeado.
...
(`SAN3-04a` → `SAN3-07` → `SAN3-18`) · `src/modules/core-saas/permissions/catalog.ts` (`SAN3-04a` → `SAN3-04b`) ·
...
aba Financeiro e o serviço de OS da web (`SAN3-01` → `SAN3-25`) · `tests/e2e/**` (`SAN3-01` → `SAN3-10`) · `src/modules/work-orders/work-order.routes.ts` (`07c` → `SAN3-26`) · `src/modules/impound/impound-prisma.repository.ts` (`SAN3-11` → `B-O6R-12`) (as duas da junta do ciclo 2, C1-A4) · textos de
...
1. **O `PLANO_O6R` só cobria os 32 achados da auditoria.** Ignorava as 231 pendências do registro e as 49 que nem
   registro tinham. **46 dos 56 bloqueantes** vêm de fora da auditoria (contados por script: 10 itens do §4.1 trazem achado Ω6R na
   coluna Item — 1, 2, 3, 7, 11, 19, 24, 27, 30 e 31; os itens 55 e 56 vêm da junta do ciclo 2).
...
7. **Conflitos mantidos e registrados (§A2), não resolvidos em silêncio:**
   ...
   - **As reclassificações `✓` do §4.1** (itens 3, 6, 7, 9, 10, 14, 15, 20–23, 25, 26, 29–31, 36, 38, 41, 46, 48, 49,
     51, 52, 53, 54, 56): as fatias as davam como não bloqueantes, condicionais ou "demo honesta, não venda"; os itens 51–54
     foram levados ao gate pela junta do ciclo 1 (C2-09, C2-12, C2-01, C3-03), e o 56 pela do ciclo 2 (lacuna da C1),
     com a decisão do dono (`D-SAN3-PLANO-OPCAO-B`). A regra do §2 as leva ao gate. A regra também
...
**Quantidade exata.** 56 bloqueantes em **37 blocos** — 11 G (`04a`, `03a`, `SAN3-02`, `SAN3-03`, `SAN3-05`, `07c`,
`SAN3-18`, `SAN3-24`, `SAN3-06b`, `SAN3-15`, `SAN3-10`), 21 M, 5 P (contados por script na coluna Esf. do §5) — mais
os 6 atos do dono e este PR. Os itens 55 e 56 (decisão do dono, opção B — §15) entram em blocos que já existiam
(`B-SAN3-12` e `B-SAN3-04a`) sem mudar o tamanho de nenhum: o melhor caso e o realista abaixo ficam **inalterados**.
...
## 15. Decisão do dono (opção B) e a aplicação

Parado no teto (`D-TETO-DOIS-CICLOS`; ciclo 2 REPROVADO 1 × 2, `agent-orchestration/omega/juntas/J-SAN3-plano-ciclo2.md`),
o PR foi ao dono pelo dossiê. **Decisão (2026-09-13, `D-SAN3-PLANO-OPCAO-B`):** caminho B — aprovado com os 5 bloqueios
do ciclo 2 como condição de entrada; **P1** — o `finance` lê OS (`work_orders:read`, pelo `B-SAN3-04a`); **P2** — o
Financeiro monta orçamento (`customers:read` e `service_catalog:read`, com a `RBAC_MATRIX.md` atualizada no mesmo bloco).
As condições estão no §5.6 (CE-G1, CE-G2, CE-1 a CE-7), marcadas por script na célula de teste de cada bloco do §5.
A primeira aplicação do §C.7 perdeu o critério `as_of` (teste (g) do `B-SAN3-10`), apontada pelo aplicador e restaurada.
