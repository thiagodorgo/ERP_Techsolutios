...
| `B-O6R-11b` · `fix/mobile-origem-da-escrita-e-lint` | a origem da escrita (`SyncOrigin`) e o lint `discarded_futures` com os 16 avisos em 11 arquivos que o `analysis_options.yaml` declara como `P-MOBILE-DISCARDED-FUTURES` | saíram do ciclo 3 do `B-O6R-11` por decisão do dono (`D-O6R-11-ESCOPO-DIVIDIDO`): **nenhum dos dois perde dado hoje** — são rede contra regressão futura, e o ciclo 3 não cabia inteiro | `mobile/flutter_app/lib/core/sync/**`, `mobile/flutter_app/analysis_options.yaml`, `mobile/flutter_app/test/**`; **`pubspec.*` proibido** | o tear-off não compila (parâmetro nomeado obrigatório) e o lint pega o descarte em função **síncrona** — a classe que o `unawaited_futures` não vê; os 16 avisos **re-medidos no analyzer do CI (3.47.5)**, não no local (3.11.4) | mobile | maioria | `B-O6R-11` ciclo 3 ✓ | M |
