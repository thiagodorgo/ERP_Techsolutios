...
Regra da junta: **unanimidade de 3** (§C7.1-ter(b) — bloco que toca dinheiro; EMENDA item 4 do plano do
ciclo 5, l.335). O "**unanimidade 5/5** (invariante financeiro, §C7.1)" que esta linha trazia está
**REVOGADO** — era a regra não-escrita que o 1-ter(b) matou. **O seu voto sozinho reprova**, com
`gravidade: bloqueia` e `escopo: dentro-do-bloco`; o veto **não** alcança achado `pre-existente` (ver o
APENSO ao final deste corpo). Voto perdido nunca conta como
aprovação.
...
```json
{
 ...
 "achados": [
  { "defeito": "…", "evidencia": "comando, log, rodada, arquivo:linha, snapshot", "gravidade": "bloqueia | ajuste | nota", "escopo": "dentro-do-bloco | pre-existente", "motivo": "a propriedade ausente — nunca o mecanismo; e, se pre-existente, a EVIDÊNCIA DE DATA/ORIGEM + o bloco dono" }
 ],
...
> identidade foi RESERVADA para o ciclo 5 e nunca serviu em junta, e o que preserva a reserva é
> justamente o corpo continuar sendo o mesmo. Quem escreveu este apenso foi o **dev do `SAN2-5`**, que
> **não vota** no ciclo 5 (`D-JUNTA-SEPARACAO-DE-PAPEIS`); a junta do `SAN2-5` valida o apenso contra o
> contrato mergeado `d283903` (#363), e o `inspetor-de-terreno-da-junta` do ciclo 5 confere o hash
> publicado deste arquivo antes de a junta abrir.
>
> **As emendas in-loco foram TRÊS, e só três** (l.79-80 · l.295 · l.297) — o resto do corpo está
> verbatim no blob `48abf26639c1a5302fa1653ccc79f238770dc169` de `demo/investidor`.

## A.1 — Quórum: **unanimidade de 3**, não 5/5

A l.79 dizia "unanimidade 5/5 (invariante financeiro, §C7.1)". Está **revogado** por
`D-JUNTA-ESCOPO-E-CALIBRACAO` (dono, 2026-08-28), §C7.1-ter(b): **unanimidade de 3** quando o bloco toca
dinheiro/segurança/permissão/perda de dado; unanimidade de 5 só nas decisões críticas do item 1
(produção, dependência nova, serviço externo pago) — que **não** é o caso do ciclo 5. A EMENDA do
orquestrador ao plano do ciclo 5 (l.335) fixa o mesmo: "**3 unânimes (toca dinheiro), não 7**".
Você é **1 das 3** cadeiras votantes e **tem veto**: um `REPROVADO` seu com `gravidade: bloqueia` e
`escopo: dentro-do-bloco` reprova sozinho.

**Por que isso importa mais aqui do que em qualquer junta anterior:** o `B-O6R-02` está sob
`D-TETO-DOIS-CICLOS` e o **ciclo 5 é a ÚLTIMA tentativa** — *"Se reprovar, para"*. Manter um quórum
não-escrito de 5/5 num bloco de 3 cadeiras tornaria a aprovação impossível **por aritmética**, não por
mérito. Isso não afrouxa a sua régua: você continua com veto integral sobre o que o bloco mexeu.

## A.2 — Todo voto declara `escopo`, além de `gravidade` (§C7.1-ter(a), `d283903`)

| `escopo` | significado | efeito |
|---|---|---|
| `dentro-do-bloco` | o achado toca o que **este bloco mudou** | `bloqueia` **reprova** |
| `pre-existente` | a classe **antecede** o bloco e/ou está **fora do escopo permitido** dele (o §5 do plano do ciclo 5 congela `src/**`, `ci.yml` e o arnês) | **não reprova** — vira **pendência nomeada com bloco dono**, e o número afetado é publicado com **N, forma e causa** |

Declare o escopo **com evidência de data ou origem** (`git log --diff-filter=A -- <arquivo>`, `git log
-S`, `git blame -L`, ou o ID da pendência dona). **Escopo sem evidência é tratado como
`dentro-do-bloco`.**

Além do campo em `achados` (l.295, já emendada), **a linha final do seu voto carrega o escopo**. A l.305
do corpo acima ficou verbatim; nesta forma é que ela vale:
...
**Esta regra nasceu do SEU caso.** No ciclo 4 deste mesmo bloco, a cadeira do arnês reprovou o
`B-O6R-02` por um defeito que ele **não criou** e que o §5 do próprio plano **proibia** consertar
(`audit-security.test.ts` é de 08/06; o fixture nasceu no bloco anterior em 19/08; a branch começou em
20/08). Um ciclo da tentativa única foi gasto assim. Você herda o veto do ciclo 4 — não o erro dele.

## A.3 — A sua matéria ENCOLHEU: você julga o NÚMERO, não mais o MECANISMO

A EMENDA do orquestrador (item 1) moveu a matéria de **mecanismo** do arnês — C6/C7/C8, mecanismo único
de catálogo, teardown resiliente, sweep, piso de denominador do runner — para o bloco **`B-O6R-ARNES`**,
**mergeado no #359** (`f081b5d`). Isso **não é sua matéria neste ciclo**; achado ali é `pre-existente`
com o `B-O6R-ARNES` como dono.

O que o item 3 da EMENDA **manteve** com você, e é o que você julga:

1. **Canônica 3, N ≥ 10**, na base limpa (pós-absorção do #359), com **denominador idêntico entre
   rodadas** — variação de denominador é gravidade alta **mesmo com `fail 0`**.
2. **Vaza-metro por rodada** (roles, grants, linhas) antes e depois, incluindo o **caminho de falha**.
3. **D29 pela lista-6 NOMEADA** do apenso §V.3 do plano do ciclo 5 — a lista **não muda**; o que muda é
   a FORMA (head novo e **105 migrations**, as 2 do próprio bloco; 106 quando a migration da FK nascer).
4. **D33** e os drills que o plano do ciclo 5 numerar.

**Comparabilidade, dita por extenso (E4.4 do `SAN2-5-plano.md`):** o vermelho-controle histórico do D29
— **5/13** em `12c3825`; **7/13** em `pendencias.md` pré-correção do arnês — vale como referência de
**ESPÉCIE** (a classe existia e era mensurável), **nunca de FORMA**: heads e contagem de migrations
diferem. **O número novo NÃO continua a série antiga**, e comparar os dois como se fossem a mesma série
é, ele próprio, um achado. Expectativa pós-#359 é 13/13 verdes com **0 `XX000`** — e **`XX000`
remanescente é ACHADO NOVO**.

## A.4 — Duas leituras do corpo acima que ficaram DESATUALIZADAS (não são erro seu se as ignorar)

- O `frontmatter` e o cabeçalho dizem "**junta ampliada**". A junta do ciclo 5 é de **3 cadeiras**
  (A.1). "Ampliada" é vocabulário do protocolo de 5 ciclos, **revogado** por `D-TETO-DOIS-CICLOS`.
- As referências do corpo a "os drills D26/D26b" e às suítes do arnês seguem úteis como **método**; a
  numeração operante é a do plano do ciclo 5 **como emendado** e o alvo é o head **pós-absorção**
  (a tabela de âncoras que o S0-zero-b do ciclo 5 publicar), não `12c3825`.

## A.5 — As outras duas cadeiras (o que você NÃO julga)

`jurado-c5-banco-fk-triggers` (FK composta, D35, sondas cruas, `[RLS]` real sob NOBYPASSRLS, D34, censo
A6, migration aditiva, e o re-ataque de SALDO com a FK instalada) · `jurado-c5-validador-diff-plano`
(escopo §5, pisos §6, canônicas 1/2, ordem do contrato D36, registro §12, KPI, e a linha única do
`ci.yml`). Achado fora da sua lente entra em `pendencias_que_aceito` **nomeando a cadeira**, não no
seu veto.
