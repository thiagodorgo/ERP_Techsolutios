...
- **Subagentes / papéis de junta:** os papéis que o Claude Code roda como subagentes isolados
  ...
  O índice e o protocolo de agentes isolados estão em **`.agents/agents/README.md`**. Invoque cada arquivo
  como a definição do subagente. A emulação sequencial pelo mesmo agente foi revogada por
  `D-JUNTA-SEPARACAO-DE-PAPEIS-TODO-FLUXO`; sem agentes isolados suficientes, a entrega bloqueia.
  **A junta e a independência das alçadas são obrigatórias (§C7).** Mantenha
...
4. **Protocolo de dificuldade — CRIAR AGENTES ANTES DE PARAR.** Reprovação/bloqueio técnico **não gera parada
   direta**. Ciclos (registro em `omega/reprovacoes/R-<entrega>-<ciclo>.md`): **ciclos 1–2 = a fábrica de
   agentes CRIA 1–2 especialistas sob medida** para o problema (entram na junta seguinte e votam); ciclo 3 =
...
segundo `app.js` de KPI) · `git grep` confirmando marcadores do bloco ·
...
| **Subagentes / papéis de junta** | agentes em `.claude/agents/*.md` (subagentes isolados) | papéis espelhados em `.agents/agents/*.md` (corpo verbatim + preâmbulo Codex); agentes isolados obrigatórios, sem emulação sequencial (§C7) |
